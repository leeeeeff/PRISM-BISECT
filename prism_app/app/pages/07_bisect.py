...
st.caption(
    ...
    "통계 증거(Tier A-DR/A-BP/B/C/D)와 생물학적 해석(BISECT)으로 계층화합니다. "
    "**Tab 1**: 통계 & 티어 배정 · **Tab 2**: 전체 케이스 목록 · "
    "**Tab 3**: 개별 케이스 BISECT 심층 분석 · **Tab 4**: 복합 프로세스 그룹"
)
...
| Tier | 방법 | 유전자 수 | 근거 수준 |
|------|------|---------|----------|
| **A-DR** | DRIMSeq+stageR 전장유전체 | 14 | quasi-DMN 회귀; 도너 단위 OFDR 동시 보정; 최고 증거 |
| **A-BP** | BISECT 도너 permutation | 5 | n=10,000 permutation; PO+SMC 양 배치 일관성 확인 |
| **B** | 독립 코호트 교차 재현 | 1 | SRA 공개 데이터에서 독립 재현 |
| **C** | 탐색적 pooled chi-square | 80 | 가성 반복 주의; 생물학적 해석 완료 |
| **D** | 통계 미지지 | 1 | 배치 불일치; 참고 목적 |
    """)
    _t1c1, _t1c2 = st.columns([1, 2])
    with _t1c1:
        _t1_tier_df = pd.DataFrame({
            "Tier": ["A-DR","A-BP","B","C","D"],
            "Count": [14, 5, 1, 80, 1],
        })
        _t1_pie = px.pie(
            _t1_tier_df, values="Count", names="Tier",
            color="Tier",
            color_discrete_map={"A-DR":"#2563eb","A-BP":"#059669","B":"#d97706","C":"#e2e8f0","D":"#dc2626"},
            title="101 케이스 Tier 분포", height=260,
        )
        _t1_pie.update_layout(margin=dict(t=40,b=10,l=10,r=10))
        st.plotly_chart(_t1_pie, use_container_width=True, key="t1_pie_main")
    with _t1c2:
        st.markdown("""
**Tier 배정 사고 과정:**
1. DRIMSeq+stageR: quasi-Dirichlet-Multinomial 회귀로 전장유전체 스캔 → **A-DR**
2. Sparse cell type에서 domain-specific 후보: 도너 permutation → **A-BP**
3. SRA 공개 데이터 교차 재현 확인 → **B**
4. pooled chi-square 탐색 (KIF21B 등) → **C**
5. 배치 불일치 (DLG1 OPC) → **D**
        """)
    st.divider()
...
with tab2:
    ...
    st.divider()
    _tier_options = ["전체", "A-DR", "A-BP", "B", "C", "D"]
    _sel_tier = st.radio("티어 필터:", _tier_options, horizontal=True, key="t2_tier_filter")
    _t2_bdf = _bdf.copy()
    if _sel_tier != "전체" and "bisect_tier" in _t2_bdf.columns:
        _t2_bdf = _t2_bdf[_t2_bdf["bisect_tier"].fillna("C") == _sel_tier]
    st.caption(f"표시 중: {len(_t2_bdf)}개 케이스 (전체 {len(_bdf)}개 중)")

    _t2_cols = {c: c for c in ["gene","cell_type","bisect_tier","m16_mechanism","domains_lost","domains_gained","stage2_pass"] if c in _t2_bdf.columns}
    _t2_renames = {"gene":"유전자","cell_type":"세포유형","bisect_tier":"Tier","m16_mechanism":"메커니즘","domains_lost":"도메인 소실","domains_gained":"도메인 획득","stage2_pass":"PASS"}
    _t2_display = _t2_bdf[[c for c in _t2_cols]].rename(columns=_t2_renames).copy()
    if "세포유형" in _t2_display.columns:
        _t2_display["세포유형"] = _t2_display["세포유형"].str.replace("_neuron","").str.replace("_"," ")

    def _t2_style(row):
        tier = row.get("Tier","C")
        colors = {"A-DR":"#dbeafe","A-BP":"#d1fae5","B":"#fef9c3","C":"","D":"#fee2e2"}
        bg = colors.get(tier,"")
        return [f"background-color:{bg}" if bg else "" for _ in row]

    st.dataframe(
        _t2_display.style.apply(_t2_style, axis=1),
        use_container_width=True,
        height=min(50 + 35 * len(_t2_display), 700),
    )

    # Mechanism distribution chart
    _mech_col = "m16_mechanism" if "m16_mechanism" in _bdf.columns else None
    if _mech_col:
        _mech_counts = _bdf[_mech_col].fillna("unknown").value_counts().head(12).reset_index()
        _mech_counts.columns = ["Mechanism","Count"]
        _fig_mech = px.bar(_mech_counts, x="Count", y="Mechanism", orientation="h",
                           color="Count", color_continuous_scale="Blues_r",
                           title="M16 자동 메커니즘 분류 분포", height=380)
        _fig_mech.update_layout(plot_bgcolor="white", margin=dict(t=40,b=10,l=10,r=10),
                                yaxis=dict(autorange="reversed"), coloraxis_showscale=False)
        st.plotly_chart(_fig_mech, use_container_width=True, key="t2_mech_bar")

with tab3:
    st.markdown(
        "**BISECT** (Biological Isoform-Switch Evidence Characterization Tool) — "
        "15개 독립 모듈(도메인 구조·STRING PPI·AlphaFold·계통보존·NMD·규제인자 등)로 "
        "기능 스위치를 다층 검증한 **101개 PASS 케이스**. "
...
with tab4:
    def _build_bisect_tier_badge(tier: str) -> str:
        _MAP = {"A-DR":("#2563eb","⭐ A-DR"),"A-BP":("#059669","✅ A-BP"),
                "B":("#d97706","🔵 B"),"C":("#6b7280","◯ C"),"D":("#dc2626","✗ D")}
        color, label = _MAP.get(str(tier).strip(), ("#6b7280", str(tier)))
        return (f"<span style='background:{color};color:white;padding:2px 8px;"
                f"border-radius:8px;font-size:0.72rem;font-weight:600'>{label}</span>")

    _PROCESS_GROUPS = [
        {
            "name": "⚡ Complex I Assembly",
            "description": ("미토콘드리아 Complex I N-module(NDUFAF5 chaperone + NDUFS4) + "
                           "Q-module(NDUFS7 PSST + NDUFS8 TYKY) — 4개 유전자 동시 수렴. "
                           "Leigh syndrome / MELAS 유전자 중복. "
...
- NDUFS8 면역침강: AD 뇌 조직에서 NDUFS8·Q-module complex 결합 약화
        ...
        {
            "name": "🧬 RNA 대사 축",
            "description": ("mRNA 수출(DDX19A DEAD-box) → 핵외분해효소(DIS3 RNB+PIN) → "
                           "탈아데닐화(CNOT11 NIC) → 핵소체 조립(NOL8 +198aa) → RNA 결합(ZCCHC17 S1 소실) — "
...
- rRNA mature form ratio: NOL8 gain-isoform → 28S/18S rRNA 비율 변화
        ...
        {
            "name": "🔧 DNA 복구 축",
            "description": ("ERCC6L2(6 SF2/SWI-SNF domains lost; ALE) + "
                           "FANCA(Fanconi_A lost; 43→11 exon) + "
                           "USP1(deubiquitylase alt-TSS +745bp) + RPS3(UTR variant) — "
...
- FANCD2 western: AD 뇌 추출물에서 FANCD2-Ub/total FANCD2 비율 감소
        ...
        },
        {
            ...
            "description": ("DOCK10(Rac1/Cdc42-GEF; 6aa minor exon; Microglia A-DR) + "
                           "DOCK11(Cdc42-GEF; 53→3 exon ALE; DHR-2+PH 전부 소실; A-BP perm_p=0.0008; phyloP=3.25) — "
...
- Spine morphology: DOCK10 KD microglia 공배양 시 dendritic spine 형태 변화
        ...
        },
        {
            ...
            "description": ("ZNF736(KRAB+C2H2 완전 소실; stageR p=2.7×10⁻²⁴) + "
                           "ZNF582(Alt-TSS +31aa; stageR p=7.2×10⁻¹¹) + "
                           "ZNF268(KRAB domain; Tier C) — "
...
- ZNF736 ChIP: CT 이소폼 ZNF736 표적 유전자 목록 작성 → AD에서 탈억제 확인
        ...
        {
            "name": "🔩 유비퀴틴-프로테아좀 축",
            "description": ("USP1(deubiquitylase) + SAMHD1(HD domain + NMD; LINE-1 restriction) + "
                           "DCAF5(WD40 E3 adaptor gain; CRL4 ligase) — "
...
- USP1 activity assay: AD 뇌 균질액에서 FANCD2-Ub 수준 정량
    ...
    st.markdown("**기능적으로 함께 움직이는 유전자 그룹** — 개별 이소폼이 아닌 수렴 축 단위로 AD 병리를 조망합니다.")
    ...
    for _pg in _PROCESS_GROUPS:
        with st.expander(f"{_pg['name']}", expanded=True):
            ...
            _pg_genes = [g.upper() for g in _pg["genes"]]
            _pg_bdf = _bdf[_bdf["gene"].str.upper().isin(_pg_genes)].copy() if "gene" in _bdf.columns else pd.DataFrame()
            if not _pg_bdf.empty:
                _pg_cols = {c: {"gene":"유전자","cell_type":"세포유형","bisect_tier":"Tier",
                                 "m16_mechanism":"메커니즘","stage2_pass":"PASS",
                                 "domains_lost":"도메인 소실","domains_gained":"도메인 획득"}.get(c,c)
                             for c in ["gene","cell_type","bisect_tier","m16_mechanism",
                                       "domains_lost","domains_gained","stage2_pass"] if c in _pg_bdf.columns}
                _pg_show = _pg_bdf[list(_pg_cols.keys())].rename(columns=_pg_cols).copy()
                if "세포유형" in _pg_show.columns:
