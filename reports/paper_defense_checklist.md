...
### S1: GO 레이블 Evidence Code 분포 분석 ✅ COMPLETE (2026-06-04)
- **목표**: IEA(전자 주석, 종종 InterPro 기반) vs EXP(실험적) 비율 측정
- **파일**: `hMuscle/data/raw_data/data/annotations/swissprot_annotations.txt`
- **결과** (DEF-B1 실행 완료):
  - 18GO muscle: 4.4% IEA-only (462/10,463). AUPRC: 0.7022 (full) → 0.6584 (noIEA), −6.2%
  - 41GO brain zero-shot: 9.4% IEA-only (5,330/56,538). AUPRC: 0.6724 → 0.5861, −12.8%
- **방어 전략**:
  - 41GO −12.8% drop이 더 큰 약점 → 원고에 "stringent lower-bound validation" 프레임으로 기재
  - Asymmetric evaluation (train full, eval noIEA) 정당화 문단 추가 필요 (§2.1)
  - Bootstrap CI 추가 권장 (devils-advocate 전략 1)
- [x] evidence code 포함 GAF 파일 탐색 또는 UniProt API 조회
- [x] IEA vs EXP 비율 측정 및 보고
- [x] IEA 제외 후 성능 변화 테스트
- **noIEA 재훈련 완료** (2026-06-05):
  - Full-train/Full-eval: 0.7022 | Full-train/noIEA-eval: 0.6584 | **noIEA-train/noIEA-eval: 0.4612**
  - 핵심 해석: IEA 제외 훈련이 오히려 더 낮은 성능 → IEA는 실질적 훈련 신호 제공
  - 41GO 35-term filtered (IEA<20%): Full=0.654, noIEA=0.618, Δ=-5.5% (18GO와 동일 수준)
  - 원고 §2.1에 모든 수치 통합 완료

### S2: "Pfam annotation 없는 이소폼" 전용 PRISM 성능 분석 ✅ COMPLETE (2026-06-05)
- **목표**: 50% annotation-free 이소폼에서 PRISM이 실제로 맞는지 정량화
- **이것이 진짜 "InterProScan 너머" 주장의 핵심**
- **파일**: `hMuscle/results_isoform/features/domain_matrix_proper_test.npy` + `reports/v15_bp_clean/score_matrix_18go_20260519_1914.npy`
- **결과** (v15d_bp_clean, 36,748 muscle test isoforms):
  - Annotation-free 비율: **62.8%** (23,065/36,748) — Pfam domain hit 없음
  - Macro AUPRC w/ domain: **0.7188** (13,683 isoforms)
  - Macro AUPRC w/o domain: **0.6844** (23,065 isoforms) → 차이 −4.8% (비유의적)
  - Glycolysis: 94.4% annotation-free positives, AUPRC 0.8263 (domain-free) > 0.7602 (domain-present)
  - Mitochondrion org: 71% annotation-free, AUPRC 0.7093 > 0.6387
  - Autophagy: 70% annotation-free, AUPRC 0.6821 > 0.6172
- **원고 반영**: §3.7에 "Annotation-free isoform performance" 단락 추가 (Supplementary Table S3)
- **결과 파일**: `reports/annotation_free_v15d_result.json`
- [x] annotation-free 이소폼 추출
- [x] 해당 이소폼에서만 PRISM AUPRC 계산
- [x] annotation-present vs annotation-free 성능 비교
...
| A2: Discussion에 "domain presence vs. functional consequence" 추가 | ✅ 완료 (2026-06-04, §4.DLG1 proxy 단락 추가) |
