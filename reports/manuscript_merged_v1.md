...
**The KIF21B motor domain switch is neuron-selective.** The CT-specific motor domain isoform (transcript293004.chr1.nic, exon-17-containing) is significantly depleted in AD exclusively in neuronal populations: excitatory neurons (χ² DTU adjusted p=1.4×10⁻¹⁰) and inhibitory neurons (p=0.034). In all non-neuronal cell types tested — oligodendrocytes, OPCs, astrocytes, microglia, vascular cells, lymphocytes — the CT isoform was undetectable across all donors, confirming that transcript293004.chr1.nic is a neuron-specific isoform in this tissue. Within neuronal populations, the effect magnitude differs: in excitatory neurons, CT isoform usage falls from 35.1% in control donors to 0% in AD (complete abolition); in inhibitory neurons, from 10.3% to 0% (p=0.034). The excitatory-neuron-dominant expression pattern is consistent with KIF21B's reported role in anterograde transport of AMPA receptor subunits to dendritic spines of glutamatergic pyramidal neurons (Morikawa et al., *eLife* 2018; PMID: 30507682).

**Pan-cellular switches (NDUFS7, DLG1) versus neuron-selective switch (KIF21B).** By contrast, the NDUFS7 Complex I isoform switch (adjusted p<0.05) was detected across four cell types: excitatory neurons, inhibitory neurons, oligodendrocytes, and astrocytes — consistent with mitochondrial ubiquity. The DLG1 OPC switch was detected in excitatory neurons, inhibitory neurons, oligodendrocytes, and OPCs, reflecting broader synaptic scaffold involvement. NDUFS8 isoform change was confined to inhibitory neurons and OPCs. This cell-type specificity gradient — ranging from ubiquitous (NDUFS7) to neuron-biased (DLG1, NDUFS8) to neuron-exclusive (KIF21B) — provides a mechanistic framework for dissecting which AD molecular events are cell-autonomous defects versus system-level consequences of circuit disruption.
