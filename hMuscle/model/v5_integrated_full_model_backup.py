# -*- coding: utf-8 -*-
# ============================================================================
# v5_integrated_full_model.py
#
# v4-3 대비 핵심 변경:
#
# [1] 전략 B: Expression Profile을 세 번째 입력 모달로 통합
#     - 입력: bambu_data/CPM_transcript.txt → log1p 변환 → (36748, 24)
#     - 아키텍처: seq_feat(16) + domain_feat(16) + expr_feat(16) = 48-dim concat
#     - feature_model EMB_DIM: 32 → 48
#     - Expression Dropout p=0.3 (train-time 정규화, train expr=zeros 대응)
#     - Training isoform: expression = zeros (bambu 데이터 없음, 생물학적으로 정당)
#     - Test isoform: expression = log1p(CPM) (실제 hMuscle SC-seq 발현값)
#
# [2] CRF 완전 제거 (EXP-01 근거: CRF가 모든 GO term에서 AUROC 평균 -0.21)
#     - Phase 3 = Focal Fine-tuning + PFN만 수행
#     - Final output = PFN score (CRF pos_prob 대신)
#     - coexp_net 로딩 제거
#
# [3] 나머지 구조 v4-3 유지
#     - Phase 1: 임베딩 기반 GradientTape Triplet (cross-gene hard negative)
#     - Phase 1.5: Linear Probing (Encoder frozen)
#     - Phase 2: Joint Focal + Ingroup Triplet
#     - PFN Bridge: TabPFNClassifier
#
# 다음 버전 예정 (v5-1):
...
EXPR_EMB_DIM = 16   # expression branch 출력 차원
EMB_DIM      = 48   # feature_model 출력 차원 (16+16+16)
EXPR_DROPOUT = 0.3  # expression branch dropout (train-time 정규화)
...
# --------------------------------------------------------------------------
# [5] 임베딩 추출 유틸 (make_batch 기반, expr 포함)
# --------------------------------------------------------------------------
def extract_embeddings(feature_model, X_seq, X_dm, X_expr, seq_dim, emb_dim=EMB_DIM):
    """
    make_batch 기반 길이별 그룹 추출.
    feature_model: [seq, domain, expr] → 48-dim concat
    """
    ...
    for key in tup_gp.keys():
        ...
        for i in range(int(len(batch_idx) / 1000) + 1):
            ...
            sub_dm   = X_dm[sub_idx]
            sub_expr = X_expr[sub_idx]
            emb[sub_idx] = feature_model.predict_on_batch([sub_seq, sub_dm, sub_expr])
    return emb
...
def build_emb_triplet_inputs(embeddings, y, pos_indices, neg_indices,
    ...
    if mode == "hard" and len(neg_indices) >= 20:
        ...
        for a_i in sampled_pos:
            ...
            _, nn_idx = nn_model.kneighbors(embeddings[a_i].reshape(1, -1))
...
def phase1_embedding_triplet_epoch(
        feature_model, X_seq, X_dm, X_expr, y, seq_dim,
        optimizer, margin=0.1, batch_size=256,
        n_batches=50, warmup=False):
    """
    임베딩 기반 Triplet GradientTape 학습 1 epoch.
    X_expr: training isoform은 zeros (expression 없음)
    """
    embeddings = extract_embeddings(feature_model, X_seq, X_dm, X_expr, seq_dim)
    ...
    for _ in range(n_batches):
        ...
        with tf.GradientTape() as tape:
            def get_live_emb(indices):
                ...
                sub_dm   = X_dm[indices]
                sub_expr = X_expr[indices]
                raw_out  = feature_model([sub_seq, sub_dm, sub_expr], training=True)
...
# [7] Phase 2 보조 함수
# --------------------------------------------------------------------------
def get_triplet_batch_ingroup(X_seq_grp, X_dm_grp, X_expr_grp, y_grp, batch_size=64):
    """
    ...
    for _ in range(actual_bs):
        ...
        a_seq.append(X_seq_grp[a_i]);   a_dm.append(X_dm_grp[a_i]);   a_expr.append(X_expr_grp[a_i])
        p_seq.append(X_seq_grp[p_i]);   p_dm.append(X_dm_grp[p_i]);   p_expr.append(X_expr_grp[p_i])
        n_seq.append(X_seq_grp[n_i]);   n_dm.append(X_dm_grp[n_i]);   n_expr.append(X_expr_grp[n_i])
        labels.append(1)

    return ([np.array(a_seq), np.array(a_dm), np.array(a_expr),
             np.array(p_seq), np.array(p_dm), np.array(p_expr),
             np.array(n_seq), np.array(n_dm), np.array(n_expr)],
            np.array(labels))


def compute_margin_stats(feature_model, X_seq, X_dm, X_expr, y, seq_dim,
                         margin=0.1, n_sample=1000):
    emb     = extract_embeddings(feature_model, X_seq, X_dm, X_expr, seq_dim)
    pos_emb = emb[y == 1]
    neg_emb = emb[y == 0]
    if len(pos_emb) < 2:
        return 0.0, 0.0
...
def save_phase_results(phase_name, model_base, X_seq, X_dm, X_expr, y_true,
                       gene_ids, iso_ids, save_dir, ver_tag):
    print("\n[Save] {}...".format(phase_name))
    preds  = model_base.predict([X_seq, X_dm, X_expr], batch_size=256, verbose=1)
...
# Training expression: zeros (hMuscle bambu 데이터 없음)
n_train_total   = len(X_train_seq)   # swissprot 포함 후 총 수
...
X_all_expr     = np.vstack([X_train_expr[:K_training_size], X_test_expr])

X_all_seq_pfn  = np.vstack([X_train_seq, X_test_seq])
X_all_dm_pfn   = np.vstack([X_train_dm, X_test_dm])
X_all_expr_pfn = np.vstack([X_train_expr, X_test_expr])
...
# upsample은 seq/dm만 처리 — expr은 train 전부 zeros이므로 별도 생성
X_train_expr_upsmp = np.zeros((len(y_train_upsmp), EXPR_DIM), dtype=np.float32)

# --------------------------------------------------------------------------
# [12] 모델 구조 (3-modal: seq + domain + expr)
# --------------------------------------------------------------------------
seq_input    = Input(shape=(None,),    dtype='int32', name='seq_input')
domain_input = Input(shape=(dm_dim,),  dtype='int32', name='domain_input')
expr_input   = Input(shape=(EXPR_DIM,), dtype='float32', name='expr_input')  # NEW
...
# Expression branch (NEW)
x3      = Dense(32, kernel_regularizer=regularizers.l2(1e-5),
                activation='relu')(expr_input)
x3      = Dropout(EXPR_DROPOUT)(x3)   # expression dropout — 정규화 핵심
expr_feat = Dense(EXPR_EMB_DIM, activation='relu')(x3)

# Fusion: 16 + 16 + 16 = 48-dim
concat = concatenate([seq_feat, domain_feat, expr_feat])
feature_model = Model([seq_input, domain_input, expr_input],
...
base_model = Model(inputs=[seq_input, domain_input, expr_input],
                   outputs=[embedding_layer, prediction_layer])
classification_model = Model(inputs=[seq_input, domain_input, expr_input],
                             outputs=prediction_layer)

# Phase 2 Triplet model (ingroup, Focal 주 / Triplet 보조)
seq_a  = Input(shape=(None,),     dtype='int32');   dm_a  = Input(shape=(dm_dim,),   dtype='int32');   expr_a = Input(shape=(EXPR_DIM,), dtype='float32')
seq_p  = Input(shape=(None,),     dtype='int32');   dm_p  = Input(shape=(dm_dim,),   dtype='int32');   expr_p = Input(shape=(EXPR_DIM,), dtype='float32')
seq_n  = Input(shape=(None,),     dtype='int32');   dm_n  = Input(shape=(dm_dim,),   dtype='int32');   expr_n = Input(shape=(EXPR_DIM,), dtype='float32')

emb_a, pred_a = base_model([seq_a, dm_a, expr_a])
emb_p, _      = base_model([seq_p, dm_p, expr_p])
emb_n, _      = base_model([seq_n, dm_n, expr_n])

triplet_loss_layer = Lambda(triplet_loss_fn, output_shape=(1,))([emb_a, emb_p, emb_n])
triplet_model = Model(
    inputs=[seq_a, dm_a, expr_a, seq_p, dm_p, expr_p, seq_n, dm_n, expr_n],
    outputs=[triplet_loss_layer, pred_a])

adam_p1   = optimizers.Adam(lr=0.0005)
adam_main = optimizers.Adam(lr=0.001)

print("\n[Model] feature_model output dim = {}".format(EMB_DIM))
print("[Model] Expression branch: Dense(32)->Dropout({})->Dense({})".format(
    EXPR_DROPOUT, EXPR_EMB_DIM))
...
save_phase_results("phase0_initial_untrained", base_model,
                   X_test_seq, X_test_dm, X_test_expr, y_test,
...
for epoch in range(PHASE1_EPOCHS):
    ...
    if (epoch + 1) % 5 == 0:
        sat, cdist = compute_margin_stats(
            feature_model, X_test_seq, X_test_dm, X_test_expr,
...
for epoch in range(2):
    ...
    for key in tup_gp.keys():
        ...
        hist = classification_model.fit(
            [X_bs[mixed], X_bd[mixed], X_be[mixed]], y_b[mixed],
...
for epoch in range(5):
    ...
    for key in tup_gp.keys():
        ...
        X_grp_expr = X_train_expr_upsmp[batch_idx]
        ...
        for step in range(max(1, int(len(batch_idx) / 64))):
            batch, labels = get_triplet_batch_ingroup(
                X_grp_seq, X_grp_dm, X_grp_expr, y_grp, batch_size=64)
            if batch is None:
                continue
...
_, p2_centroid_dist = compute_margin_stats(
    feature_model, X_test_seq, X_test_dm, X_test_expr, y_test, seq_dim, margin=0.1)

# ==========================================================================
# PHASE 3: Focal Fine-tuning + PFN (CRF 제거)
# EXP-01 근거: CRF가 모든 GO term에서 AUROC 평균 -0.21 감소
# Phase 3 Final = PFN score (CRF 없음)
# ==========================================================================
print("\n" + "=" * 50)
print(">>> PHASE 3: Focal Fine-tuning + PFN (no CRF)")
print("=" * 50)
...
for epoch in range(phase3_epochs):
    ...
    # 3-2. PFN 임베딩 추출 (train + test, expr 포함)
    print("  Extracting embeddings for PFN...")
    X_pfn_emb = extract_embeddings(
        feature_model, X_all_seq_pfn, X_all_dm_pfn, X_all_expr_pfn, seq_dim)
    ...
    test_pfn = pfn_score_all[len(y_train):]
    print("  PFN → mean={:.4f} std={:.4f} >0.5={} >0.3={}".format(
        test_pfn.mean(), test_pfn.std(),
        (test_pfn > 0.5).sum(), (test_pfn > 0.3).sum()))

    # 3-4. Label update (CRF 역할 대체 — PFN 점수 기반 간이 업데이트)
    if epoch < phase3_epochs - 1:
        train_pfn = pfn_score_all[:K_training_size]
        # PFN 점수로 train label 업데이트 (threshold 0.5)
        y_train_pfn_update = (train_pfn > 0.5).astype(int)
        # MIL 제약: positive bag은 최소 1개 positive 유지
        for bag_i in range(int(np.max(crf_bag_index))):
            indx = np.where(crf_bag_index[:K_training_size] == bag_i)[0]
            if len(indx) == 0:
                continue
            bag_lbl = np.max(y_all[:K_training_size][indx])
            if bag_lbl == 1 and y_train_pfn_update[indx].sum() == 0:
                best = np.argmax(train_pfn[indx])
                y_train_pfn_update[indx[best]] = 1
            elif bag_lbl == 0:
                y_train_pfn_update[indx] = 0
        y_train[:K_training_size] = y_train_pfn_update

        X_train_seq_upsmp, X_train_dm_upsmp, y_train_upsmp, unused_flag = upsample(
            y_train, gene_index, gene_count, X_train_seq, X_train_dm, unused_flag)
        X_train_expr_upsmp = np.zeros((len(y_train_upsmp), EXPR_DIM), dtype=np.float32)

# Final PFN score (CRF 없음)
