# -*- coding: utf-8 -*-
# ============================================================================
# v5-2_integrated_full_model.py
#
# v5-1 대비 변경 (5가지 — 일반성 유지 원칙):
#
# [I1] REMOVED — prior bias init (logit=-4~-6) froze all outputs near 0.
#     Focal loss (1-p_t)^γ zeroed negative gradients when p≈pos_ratio,
#     making Phase 2 unable to learn. v5-1 mode-collapse was score
#     distribution artifact only; AUROC/AUPRC were unaffected.
#
# [I2] LabelProp alpha 선택 기준: AUROC → AUPRC  [R9.1]
#     - 문제: AUROC 기준 선택 시 GO_0006412 AUPRC -0.062 역효과
#     - 수정: average_precision_score로 best alpha 선택
#     - 일반성: sparse class (pos < 5%) 모두 해당, 규칙 [R9.1] 준수
#
# [I3] Phase 1 triplet margin: 0.1 → 0.3  [R3.1]
#     - 문제: L2-norm 공간에서 semi-hard zone이 16-31%로 좁음
#             → gradient 신호 부족, 빠른 수렴 후 stagnation
#     - 수정: margin=0.3 → semi-hard zone 36-46%로 확대
#     - 일반성: [R3.1] default margin, GO term 무관
#
# [I4] Phase 1 n_batches: categorical 분기 → coverage 공식
#     - 문제: if/elif/else (30/50/100)는 4개 GO term 관찰 후 수동 설계
#     - 수정: n_batches = max(20, min(150, ceil(n_pos * 0.8 / batch_size)))
#     - 일반성: positive density에 비례, 어떤 GO term도 동일 공식 적용
#
# [I5] Phase 2 AUROC 체크 간격: every 3 → every 1 epoch
#     - 문제: 3 epoch 간격으로 best checkpoint를 놓칠 수 있음
#     - 수정: CHECK_EVERY=1, NO_IMPROVE_LIMIT=3
#     - 일반성: 모든 GO term에서 early stop 정확도 향상
#
# 신규 추가:
# [EMB] Phase별 임베딩 공간 정량 분석
#     - Silhouette score (cosine), Intra/Inter distance ratio,
#       Centroid distance, Linear separability (LogReg AUROC)
#     - UMAP 시각화 (4 phase overlay)
# ============================================================================
import numpy as np
import sys
import os
...
EXPR_DIM = 24
EMB_DIM  = 32
MARGIN_P1 = 0.3        # [I3] 0.1 → 0.3 (R3.1)
COVERAGE_TARGET = 0.8  # [I4] positive당 epoch당 기대 anchor 횟수
...
# --------------------------------------------------------------------------
# [I4] Phase 1 n_batches: coverage-based formula (GO-specific 분기 제거)
# --------------------------------------------------------------------------
BATCH_SIZE_P1 = 256
N_BATCHES_P1  = max(20, min(150, int(np.ceil(n_pos_train * COVERAGE_TARGET / BATCH_SIZE_P1))))
coverage_per_epoch = N_BATCHES_P1 * BATCH_SIZE_P1 / n_pos_train
print("[I4] n_batches={} (n_pos={}, coverage={:.1f}x/epoch, formula-based)".format(
