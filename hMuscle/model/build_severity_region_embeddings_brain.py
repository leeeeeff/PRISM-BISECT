#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""build_severity_region_embeddings_brain.py

Brain extension of build_severity_region_embeddings_muscle.py (2026-07-21,
user-selected Option C follow-up: after the muscle pilot produced several
predictions-falsifying results -- size_z/domain_binary amplified under
region-pool, nterm_overlap COLLAPSED under region-pool while its scrambled-
window control did not -- test whether this pattern is muscle-specific or
general, before deciding on further experiment direction. See
muscle_severity_pairs_region.tsv / analyze_severity_region_covariates.py
output for the muscle reference numbers this run will be compared against.

IDENTICAL design/guards to the muscle script (C1 scrambled null, C2 fixed
mean-pool-trained coherence direction, L15+L30 only, per-residue tensors
discarded after per-pair pooling) -- only the tissue-specific data sources
change:
  - pair table: reports/severity_pairs/brain_severity_pairs_scored.tsv
    (33,802 pairs / 9,958 genes, vs muscle's 15,885/6,001)
  - isoform ids: brain_isoquant_esm2/full/brain_full_ids.npy (NOT
    my_isoform_list_fixed.npy -- brain uses a different id space, mixing
    GENCODE ENST ids, long-read novel-transcript ids, and gene-symbol-based
    ids; see the ID-join feasibility check earlier this session)
  - sequences: reports/truebrain_rerun_20260714/data/brain_full_proteins.fa
    via build_severity_pairs.parse_fasta_sequences
  - mean-pool L15/L30: hMuscle/data/brain_isoquant_esm2/full/
    brain_full_esm2_layer{15,30}_t30_150M.npy
"""
...
IN_TSV = OUT_DIR / 'brain_severity_pairs_scored.tsv'
OUT_TSV = OUT_DIR / 'brain_severity_pairs_region.tsv'
EMB_CACHE = OUT_DIR / 'brain_region_embeddings.npz'
MAXLEN = 1022
N_LAYERS_KEEP = (15, 30)
...
def load_pairs_and_seqs():
    ...
    print(f"[load] {len(df)} brain pairs, {len(iso_b)} isoform ids, {len(seqs)} sequences", flush=True)
    return df, iso_b, seqs


def build_occurrences(df, iso_b, seqs):
    keep_rows = []
    iso_seq = {}
    occ = {}
    ivs_size = {}
    ...
    for row_idx, r in df.iterrows():
        long_id = iso_b[int(r['long_idx'])]
        short_id = iso_b[int(r['short_idx'])]
...
def main():
    df, iso_b, seqs = load_pairs_and_seqs()
    ...
    keep_rows, iso_seq, occ, ivs_size = build_occurrences(df, iso_b, seqs)

    smoke_limit = os.environ.get('EMB_SMOKE_LIMIT')
    if smoke_limit:
        smoke_limit = int(smoke_limit)
        keep_rows = keep_rows[:smoke_limit]
        keep_ids = {iso_b[int(df.loc[r]['long_idx'])] for r in keep_rows} | \
                   {iso_b[int(df.loc[r]['short_idx'])] for r in keep_rows}
