...
# -*- coding: utf-8 -*-
"""
bisect_prism_score_comparison_v2_truebrain.py  (Option B)
------------------------------------------------------------
TISSUE-MISLABELING BUGFIX RERUN (2026-07-14).
Original bisect_prism_score_comparison.py loaded v17f_star_preds.npy (computed on MUSCLE
data via v17f_star_bootstrap_ci.py) and my_gene_list_fixed.npy (MUSCLE gene list, ENSG IDs)
-- despite being described as a "brain" isoform-discrimination check for BISECT genes.
This rerun points PRED_DIR at the TRUE-brain v17f* predictions produced by
v17f_star_bootstrap_ci_v2_truebrain.py (reports/truebrain_rerun_20260714/v17f_star_bootstrap/),
and the gene list at brain_full_gene_names.npy (TRUE brain, 63994 isoforms / 18514 unique
genes, IsoQuant IDs). Critical difference: brain gene names are already SYMBOLS (e.g. 'A1BG'),
not ENSG IDs -- so BISECT target lookup is switched from ensg2idxs (ENSG-keyed) to
gene2idxs (symbol-keyed) directly; SYM2ENSG is retained only for display/cross-reference.
BISECT itself already operates on true brain data (the 83 case set is unaffected) -- only
the PRISM-score side needed correcting.

Original (mislabeled-as-brain, actually muscle): bisect_prism_score_comparison.py, backed up
at bisect_prism_score_comparison_backup_20260714.py before this rerun.
...
PRED_DIR  = '../../reports/truebrain_rerun_20260714/v17f_star_bootstrap'
OUT_DIR   = '../../reports/truebrain_rerun_20260714/bisect_prism_scores'
...
# ── 1. Load saved v17f* predictions (TRUE BRAIN) ──────────────────
print("\n[1] Loading saved v17f* predictions (TRUE BRAIN)...")
preds = np.load(f'{PRED_DIR}/v17f_star_preds.npy')   # (63994, 82)
Y_te  = np.load(f'{PRED_DIR}/Y_te.npy')               # (63994, 82)
...
# te_ensg_list derived FROM symbols via SYM2ENSG (display/cross-ref only) -- brain data
# has no native ENSG IDs, so this is best-effort and may be empty for some symbols.
te_ensg_list = [SYM2ENSG.get(s, s) for s in te_sym_list]
...
# GO term name lookup.
# BUGFIX (pre-existing in original muscle script too, identical code, confirmed via diff):
# go-basic.obo.gz does NOT exist anywhere on this filesystem (confirmed via filesystem search),
# so the original try/except silently left go_names={} empty, which made
# find_relevant_go_idxs() below match keywords against raw GO IDs (e.g. 'GO:0016491') instead
# of human-readable names -- meaning it NEVER found any keyword hit, on muscle OR brain, in any
# prior run of this script. Fixed by building go_names from mf_domain_vs_prism.tsv's own
# 'name' column (already loaded/used elsewhere in this exact pipeline for the same 82 MF terms),
# which IS present and correct -- no external OBO file needed.
go_names = {}
with open('../../reports/v_expanded_gomf/mf_domain_vs_prism.tsv') as f:
    next(f)
    for line in f:
        p = line.strip().split('\t')
        if len(p) >= 2: go_names[p[0]] = p[1]
print(f"  GO term names loaded from mf_domain_vs_prism.tsv: {len(go_names)}")
...
for gene, info in BISECT_TARGETS.items():
    ensg    = info['ensg']
    # TRUE BRAIN fix: look up by gene SYMBOL (native key for brain_full_gene_names.npy),
    # not ENSG (brain data was never ENSG-keyed; the muscle-mislabeled original's
    # ensg2idxs lookup would silently return [] for every BISECT gene on brain data).
    idxs    = gene2idxs.get(gene, [])
