# -*- coding: utf-8 -*-
"""
v_expanded_gomf_v2_truebrain.py
================================
TISSUE-MISLABELING BUGFIX RERUN (2026-07-18, line-310 decision).

Original v_expanded_gomf.py's "brain zero-shot" evaluation (279-term BP+MF+CC,
H1/H2/H3 failure classification, transfer-zone classification, pos_bias) loads
`my_gene_list_fixed.npy` / `my_isoform_list_fixed.npy` as its test set -- this is
the MUSCLE held-out fold (36,748 isoforms), not brain. Same bug class as
[[session_20260714_muscle_brain_mislabel]] (82-MF / Domain LR / etc.), but this
script was NOT covered by that rerun batch -- only brain_domain_lr_v2_truebrain.py
(Domain LR baseline alone) was redone. The full H1/H2/H3 tissue-bias test, which
explicitly compares "muscle" vs "brain" AUPRC, was never actually run against a
different tissue; it was muscle-val-fold vs muscle-heldout-fold. This rerun
re-points the TEST side at the TRUE brain isoform set (63,994 isoforms,
brain_isoquant_esm2/full/) so the H1 (tissue bias) hypothesis is tested against
real cross-tissue data.

Training side (train_gene_list.npy, esm2_train_human_t30_150M.npy) UNCHANGED --
same 31,668-isoform muscle training set, identical hyperparameters (focal γ=2.0,
5 seeds) as original.

Original (mislabeled-as-brain, actually muscle): v_expanded_gomf.py -- left
unmodified (not overwritten; this is a new sibling file per architecture.md
...
print("="*70)
print("  PRISM Expanded GO: BP + MF + CC (gene2go) -- TRUE BRAIN rerun")
...
# ── Step 2: training gene list 로드 및 ID 변환 (UNCHANGED, muscle) ───────────
...
print(f"  Training genes (muscle, unchanged): {len(tr_id_set):,}")

# ── Step 3: gene2go 파싱 → GO term별 positive gene set (training-restricted) ─
go_info    = {}   # go_id → {'name': str, 'cat': str ('BP'/'MF'/'CC')}
go_genes   = defaultdict(set)  # go_id → set of training gene IDs
go_genes_all = defaultdict(set)  # go_id → set of ALL human gene IDs (for true-brain test labels)
...
# ── Step 4: GO term 선택 (n_positive >= MIN_POS, training-side count, tissue-independent) ──
selected_terms = []
...
# ── Step 5: ESM-2 임베딩 로드 (train=muscle unchanged, test=TRUE BRAIN) ──────
...
print(f"  Train (muscle): {X_tr.shape}  Test (TRUE BRAIN): {X_te.shape}")
...
te_sym_list = load_ids(f'{BRAIN_DIR}/brain_full_gene_names.npy')
te_iso_list = load_ids(f'{BRAIN_DIR}/brain_full_ids.npy')
print(f"  Test genes (TRUE BRAIN): {len(te_sym_list)}  isoforms: {len(te_iso_list)}")
...
# ── Step 6: 훈련 레이블 행렬 구성 (muscle, unchanged) ────────────────────────
...
def build_labels_test_truebrain(go_id):
    # TRUE BRAIN test labels: use the FULL (not training-restricted) gene2go positive
    # set, matched via symbol → Entrez ID, mirroring brain_domain_lr_v2_truebrain.py's
    # build_labels_test_brain. This is the tissue-independent ground truth for
    # whichever gene appears in the brain isoform set (18,514 genes), not just the
    # 12,709 muscle test genes the original (mislabeled) script covered.
...
Y_te = np.stack([build_labels_test_truebrain(go_id) for go_id in selected_terms], axis=1)
...
print(f"  Y_te (TRUE BRAIN) shape: {Y_te.shape}  positives/term: "
      f"min={Y_te.sum(0).min():.0f} median={np.median(Y_te.sum(0)):.0f} "
      f"max={Y_te.sum(0).max():.0f}")

# ── Step 7: PRISM MLP 훈련 (muscle, identical hyperparameters to original) ──
...
print(f"\n  Training PRISM ({N_GO} GO terms, {N_SEEDS} seeds) on muscle...")
t0 = time.time()
...
for seed in range(N_SEEDS):
    ...
    seed_preds_te.append(m.predict(X_te, batch_size=1024, verbose=0))  # TRUE BRAIN
    print(f"    seed {seed} done")

preds_te = np.mean(seed_preds_te, axis=0)  # (N_brain_isoforms, N_GO)
print(f"  Training done: {time.time()-t0:.0f}s")

# 훈련 세트 예측 (muscle held-out validation fold, still called "muscle" here)
...
# ── Step 8: ESM-2 LR baseline (muscle-trained, evaluated on TRUE BRAIN test) ─
...
for i, go_id in enumerate(selected_terms):
    ...
    results.append({
        ...
        'prism_brain':  round(prism_te,  4),   # TRUE brain now
        'lr_muscle':    round(lr_val,    4),
        'lr_brain':     round(lr_te,     4),   # TRUE brain now
...
df.to_csv(f'{OUT_DIR}/expanded_go_per_term_truebrain.tsv', sep='\t', index=False)

print(f"\n{'='*70}")
print(f"  Results (TRUE BRAIN): {len(df)} GO terms")
print(f"{'='*70}")
...
for cat in ['BP', 'MF', 'CC']:
    ...
    print(f"    TRUE Brain   — PRISM: {b_prism:.4f}  LR: {b_lr:.4f}  Δ(MLP): {b_delta:+.4f}")
    print(f"    TRUE Brain n>0: {len(b_sub)} terms, AUPRC>0.5: {b_n50}")

# ── Step 11: Transfer 성공/실패 분류 (against TRUE brain) ───────────────────
print(f"\n{'='*70}")
print(f"  Transfer Zone Classification (TRUE brain zero-shot)")
print(f"{'='*70}")
...
# ── Step 12: H1/H2/H3 classification (against TRUE brain) ──────────────────
print(f"\n{'='*70}")
print(f"  H1/H2/H3 Failure Classification (TRUE brain)")
print(f"{'='*70}")
h1 = df_brain[(df_brain['prism_muscle'] > 0.5) & (df_brain['prism_brain'] < 0.4)]
h2 = df_brain[(df_brain['prism_muscle'] < 0.5) & (df_brain['prism_brain'] < 0.5)]
from scipy.stats import spearmanr
h3_rho, h3_p = spearmanr(df_brain['n_pos_te'], df_brain['prism_brain'])
print(f"  H1 (tissue bias):         n={len(h1)}/{len(df_brain)} ({100*len(h1)/len(df_brain):.1f}%)")
print(f"  H2 (ESM-2 hard):          n={len(h2)}/{len(df_brain)} ({100*len(h2)/len(df_brain):.1f}%)")
print(f"  H3 (n_pos size, Spearman ρ={h3_rho:.3f}, p={h3_p:.2e})")
print(f"  MLP_hurts (MLP < LR by >0.05): n={(df_brain['delta_brain'] < -0.05).sum()}/{len(df_brain)}")

# ── Step 13: 전체 결과 저장 ──────────────────────────────────────────────────
summary = {
    'config': {'min_pos': MIN_POS, 'n_go': N_GO, 'n_seeds': N_SEEDS,
               'cat_counts': dict(cat_counts), 'test_set': 'TRUE_BRAIN_63994'},
    'macro': {
        cat: {
    ...
    'h1_h2_h3': {
        'n_evaluated': int(len(df_brain)),
        'h1_tissue_bias_n': int(len(h1)),
        'h2_esm2_hard_n': int(len(h2)),
        'h3_spearman_rho': float(h3_rho),
        'h3_spearman_p': float(h3_p),
        'mlp_hurts_n': int((df_brain['delta_brain'] < -0.05).sum()),
    },
    'timestamp': time.strftime('%Y-%m-%d %H:%M')
}
with open(f'{OUT_DIR}/expanded_go_summary_truebrain.json', 'w') as f:
    json.dump(summary, f, indent=2)

np.save(f'{OUT_DIR}/preds_truebrain_expanded.npy', preds_te)

print(f"\n  Saved:")
print(f"    {OUT_DIR}/expanded_go_per_term_truebrain.tsv")
print(f"    {OUT_DIR}/expanded_go_summary_truebrain.json")
print(f"    {OUT_DIR}/preds_truebrain_expanded.npy")
print(f"\n{'='*70}")
print(f"  DONE")
print(f"{'='*70}")
