# -*- coding: utf-8 -*-
# ============================================================================
# v5-4_integrated_full_model.py
#
# v5-3 대비 변경 (v5-3 실험 결과 반영):
#
# [I1] REMOVED — prior bias init 제거 유지 (v5-3 동일)
#
# [I2] LabelProp alpha 선택 기준: AUPRC [R9.1]  ← 유지
#
# [I3] margin=0.1 → 0.3 복구  ← v5-3 진단 오류 수정
#     - v5-3 실험: margin=0.1이 sparse class Silhouette 개선에도 AUPRC 전면 하락
#       GO_0006936: AUPRC 0.41→0.12, GO_0006096: AUPRC 0.35→0.10
#     - v5-2(margin=0.3, n_batches=20): Ph1 Silhouette=0.47, AUPRC=0.41
#       v5-3(margin=0.1, n_batches=50): Ph1 Silhouette=0.04, AUPRC=0.12
#     - margin=0.3의 "frustrated triplet" (sat=0%)이 더 강한 gradient 제공
#     - n_batches=50 유지: margin=0.3 + 50 batch 조합 첫 실험
#     - STREAK_LIMIT=4 유지 (margin=0.3 수렴 느리므로)
#
# [I4] n_batches 공식: max(50, min(150, ceil(n_pos × 4 / 256)))  ← 유지
#
# [I5] Phase 2 early stop 기준: AUROC → AUPRC  ← 신규 [R9.1]
#     - sparse class에서 AUPRC가 primary metric
#     - GO_0006412 positive collapse 완화 목적 (AUROC 기준은 93% positive 예측도 방치)
#     - best checkpoint 저장 기준도 AUPRC로 교체
#
...
try:
    ...
    print("Usage: python v5-4_integrated_full_model.py <GO_TERM>")
    sys.exit(1)
...
EXPR_DIM = 24
EMB_DIM  = 32
MARGIN_P1 = 0.3        # [I3 복구] 0.1 → 0.3 (v5-3 진단 오류: AUPRC 전면 하락 확인)
...
print("[Model] Phase 1 margin={:.1f} [I3 복구] | n_batches={} [I4]".format(MARGIN_P1, N_BATCHES_P1))
...
# ==========================================================================
print("\n" + "=" * 50)
print(">>> PHASE 1: Embedding-based Triplet (margin={:.1f} [I3 복구], max 15 epochs)".format(MARGIN_P1))
print("=" * 50)
