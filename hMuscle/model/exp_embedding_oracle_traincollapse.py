...
OUT_DIR   = '../../reports/truebrain_rerun_20260714/exp_embedding_oracle_traincollapse'
...
scaler = MaxAbsScaler(); delta_tr_s = scaler.fit_transform(delta_tr).astype(np.float32)  # collapse applied after tr_gi built (see below)
...
tr_gi = np.unique(np.array(tr_genes), return_inverse=True)[1]
...
# TRAIN-TIME COLLAPSE: replace each training isoform's features with its gene-mean
def _gmc(feat, gi):
    K=gi.max()+1; s=np.zeros((K,feat.shape[1]),np.float64); np.add.at(s,gi,feat)
    c=np.bincount(gi,minlength=K).astype(np.float64); return (s/c[:,None])[gi].astype(np.float32)
delta_tr_s = _gmc(delta_tr_s, tr_gi); X_tr_l30 = _gmc(X_tr_l30, tr_gi)
print('[collapse] training features replaced with gene-mean (train-time oracle)', flush=True)
...
print(" sanity: muscle normal≈0.734, muscle TRAIN-collapse oracle≈0.803 (recipe validation);")
