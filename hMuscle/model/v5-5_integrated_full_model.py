# -*- coding: utf-8 -*-
# ============================================================================
# v5-5_integrated_full_model.py
#
# v5-4 대비 변경 (v5-4 실험 결과 반영):
#
# [I1] REMOVED — prior bias init 제거 유지 (동일)
#
# [I2] LabelProp alpha 선택 기준: AUPRC [R9.1]  ← 유지
#
# [I3] margin=0.3 유지  ← v5-4 확인: margin=0.3이 전 GO term에서 우월
#     - v5-2/v5-4 공통: margin=0.3이 margin=0.1(v5-3) 대비 전체 AUPRC 우위
#     - "frustrated triplet" 효과 유지 필요 (sat≈0~30%에서 강한 gradient)
#
# [I4] n_batches — Coverage 기반 동적 할당  ← 신규 (v5-4에서 핵심 교체)
#     - 문제: v5-4 공식 max(50, ...) → 전 GO term이 n=50으로 클램핑
#         GO_0006096(n_pos=287): coverage=44.6x/epoch → 과도한 반복 → 과적합
#         GO_0006936(n_pos=840): coverage=15.2x/epoch → v5-2(6.1x) 대비 과다
#         GO_0022900(n_pos=1027): coverage=12.5x/epoch
#         GO_0006412(n_pos=3046): coverage=4.2x/epoch → 이 term만 n=50이 적합
#     - 진단: v5-4에서 GO_0006936 AUPRC=0.191 (v5-2=0.410 미달)
#             v5-2의 최고 성능은 GO_0006936 coverage=6.1x(n=20)에서 달성
#     - 해결: TARGET_COVERAGE=6.0 기준으로 GO term별 n_batches 자동 결정
#         공식: n_batches = clip(ceil(n_pos × 6.0 / 256), min=20, max=50)
#         GO_0006096(287): ceil(6.7)=7  → clip → 20  (coverage=17.8x)
#         GO_0006936(840): ceil(19.7)=20 → clip → 20  (coverage=6.1x)  ★v5-2 재현
#         GO_0022900(1027): ceil(24.1)=25 → clip → 25  (coverage=6.2x)
#         GO_0006412(3046): ceil(71.4)=72 → clip → 50  (coverage=4.2x, 상한)
#     - 범용성: 새 GO term 추가 시 코드 수정 없이 자동 적용
#
# [I5] Phase 2 early stop 기준: AUPRC  ← 유지 [R9.1]
#
...
try:
    ...
    print("Usage: python v5-5_integrated_full_model.py <GO_TERM>")
    sys.exit(1)
...
EXPR_DIM = 24
EMB_DIM  = 32
MARGIN_P1         = 0.3   # [I3] 유지: frustrated triplet → 지속적 gradient
TARGET_COVERAGE   = 6.0   # [I4] epoch당 목표 coverage배수 (v5-2 최적값 기준: GO_0006936 6.1x → AUPRC=0.41)
...
#
# 문제 (v5-3/v5-4): max(50, ...) 공식 → 전 GO term이 n=50으로 클램핑
#   GO_0006096(n_pos=287): coverage=44.6x → 과도한 반복 → 과적합
#   GO_0006936(n_pos=840): coverage=15.2x → v5-2 최적(6.1x) 대비 과다
#
# 해결: epoch당 양성 샘플 coverage를 TARGET_COVERAGE(6.0x)로 통일
#   n_batches = clip(ceil(n_pos × TARGET_COVERAGE / BATCH_SIZE), min=20, max=50)
#
#   GO_0006096(287): clip(7,  20, 50) = 20  → coverage=17.8x (하한 적용)
#   GO_0006936(840): clip(20, 20, 50) = 20  → coverage=6.1x  ★v5-2 조건 재현
#   GO_0022900(1027):clip(25, 20, 50) = 25  → coverage=6.2x
#   GO_0006412(3046):clip(72, 20, 50) = 50  → coverage=4.2x  (상한 적용)
# --------------------------------------------------------------------------
BATCH_SIZE_P1 = 256
...
# ==========================================================================
print("\n" + "=" * 50)
print(">>> PHASE 1: Embedding-based Triplet (margin={:.1f} [I3], n_batches={} [I4-dynamic], max 15 epochs)".format(
...
best_phase2_auprc  = 0.0      # [I5] AUROC → AUPRC [R9.1]
...
for epoch in range(PHASE2_MAX_EPOCHS):
    ...
    # [I5] 매 epoch AUPRC 체크 + best checkpoint [R9.1]
