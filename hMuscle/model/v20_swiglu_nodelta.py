# -*- coding: utf-8 -*-
"""
v20_swiglu_nodelta.py
======================
Ablation: v19 에서 Delta embedding 제거, SwiGLU+Residual만 유지

목적:
  v19(SwiGLU + Residual + Delta) vs v15d_bp_clean(ReLU MLP) 에서
  v19가 전 term -0.138 하락한 원인 분리.

  가설:
    (A) Delta embedding이 유용한 gene-mean 신호를 제거해 성능 하락
    (B) SwiGLU 구조 자체가 이 task에 맞지 않음

  이 실험: Delta 제거 → (A) vs (B) 분리
    - v20 ≈ v15d_bp_clean → (A) 확정: delta가 원인
    - v20 << v15d_bp_clean → (B) 확정: SwiGLU 구조 자체 문제

Architecture (v20 = SwiGLU only, 640d input):
  Input: ESM-2 640d
  → SwiGLU_Block(640→512) + Residual
  → SwiGLU_Block(512→256) + Residual
  → Dense(128, relu) → Dense(64, relu)
  → Dense(1, sigmoid)

vs v19 (1280d input = 640 ESM-2 + 640 delta):
  Delta 제거로 입력 차원 1280 → 640 복원
"""
...
OUT_DIR   = '../../reports/v20_swiglu_nodelta'
...
print("=" * 70)
print("  v20 SwiGLU + Residual (NO Delta) — ablation vs v19")
print("=" * 70)
...
# 640d input only (no delta concat)
...
print(f"  Input: 640d (ESM-2 only, NO delta)")
print(f"  GO terms: {N_GO} (same 18 BP terms as v15d_bp_clean/v19)")

# ── SwiGLU block (same as v19) ────────────────────────────────────────────────
def swiglu_residual_block(x, dim, dropout_rate=0.3):
    in_dim = x.shape[-1]
    gate  = layers.Dense(dim)(x)
    value = layers.Dense(dim)(x)
    out   = layers.Activation('swish')(gate) * value
    out   = layers.LayerNormalization()(out)
    shortcut = layers.Dense(dim, use_bias=False)(x) if in_dim != dim else x
    out = out + shortcut
    out = layers.Dropout(dropout_rate)(out)
    return out

def build_v20():
    inp = layers.Input(shape=(640,))              # ESM-2 640d only
    x = swiglu_residual_block(inp, 512, 0.3)      # 640 → 512
...
print(f"  v15d_bp_clean baseline:    0.7022")
print(f"  v19_swiglu+delta:          (pending)")
...
print(f"\n  Reversal genes: {len(top_rev)}  (v15d_bp_clean baseline: 132)")

# ── Save ──────────────────────────────────────────────────────────────────────
from datetime import datetime
ts = datetime.now().strftime('%Y%m%d_%H%M')

np.save(f'{OUT_DIR}/score_matrix_v20_{ts}.npy', score_matrix)
result = {
    'timestamp': ts, 'model': 'v20_swiglu_residual_nodelta',
    'ablation': 'delta_embedding_removed',
    'arch': {'activation': 'SwiGLU', 'residual': True, 'delta_embedding': False,
             'input_dim': 640, 'hidden': [512, 256, 128, 64]},
    ...
    'baselines': {'v15d_bp_clean': 0.7022, 'v19_swiglu_delta': None, 'v10b': 0.6935},
    'n_reversal_genes': len(top_rev),
    'top20_reversals': top_rev[:20],
}
json.dump(result, open(f'{OUT_DIR}/v20_results_{ts}.json', 'w'), indent=2)
print(f"\n  [Saved] {OUT_DIR}/v20_results_{ts}.json")
print(f"  [Saved] {OUT_DIR}/score_matrix_v20_{ts}.npy")
print("\nALL DONE")
