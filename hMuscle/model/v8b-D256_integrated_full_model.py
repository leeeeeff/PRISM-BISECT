# -*- coding: utf-8 -*-
# ============================================================================
# v8b-D256_integrated_full_model.py  (v8b + EMB_DIM 64→256, SwissProt 유지)
#
# v8b 대비 변경사항:
#   ESM-2 projection bottleneck 확장: 64-dim → 256-dim
#   SwissProt 학습 데이터 유지 (P3와 달리 제거 안 함)
#
#   근거: esm2_dim_sweep.py 결과
#     PCA 64d=35.4%, PCA 256d=64.3%, PCA 512d=91.0% (ceiling 대비)
#     v8b 64-dim contrastive (0.357) > PCA 256-dim (0.331)
#     → contrastive 1.95× PCA 효율 → 256-dim contrastive 잠재력 높음
#
#   P3 실험 교훈:
#     SwissProt 완전 제거 → GO:0006096 AUPRC 0.7945→0.4640 (human positive 32개, 87.6% 의존도)
#     → SwissProt은 반드시 유지
#
#   2×2 ablation design:
...
#
# 변경 내역 (v8b 대비):
#   esm2_d2:      Dense(128) → Dense(256)
#   esm2_feat:    Dense(64)  → Dense(256)   ← 핵심 bottleneck 해소
#   head_dense64: Dense(64)  → Dense(256)   이름: head_dense256
#   emb_dim:      64 → 256
#   concat:       [64+32+16+16=128] → [256+32+16+16=320]
#   w_init:       reshape(64,1) → reshape(256,1)
#
# [ARCH] v8b-D256
#   ESM-2: 640→256 | CNN: 1500→32 | Domain: LSTM16 | DomainDelta: 251→16
#   Fusion: concat[320] → Dense(256) → L2_norm → emb[256] → Dense(1,sigmoid)
#
# [TRAIN] v8b 동일
#   Phase 1 (15 ep): ESM-2+Domain train, CNN frozen
#   Phase 2 (25 ep): Unified(α=0.2) + 5그룹 discriminative LR
#   slow LR 대상: ESM-2 + head_dense256 (geometry 보호)
# ============================================================================
import numpy as np
import sys
import os
...
HEAD_DENSE_NAMES      = {'head_dense256'}          # [D256] embedding geometry — slow LR
...
def prototype_warm_init(base_model, embeddings_np, y_np, proto_fn=None,
    ...
    for layer in base_model.layers:
        if layer.name == 'prediction_out':
            w_init = (w_dir * scale).reshape(256, 1).astype(np.float32)  # [D256] 64→256
...
x_esm = Dense(256, kernel_regularizer=regularizers.l2(1e-5), name='esm2_d2')(x_esm)  # [D256] 128→256
x_esm = Activation('relu')(x_esm)
esm2_feat  = Dense(256, activation='relu', kernel_regularizer=regularizers.l2(1e-5),
                   name='esm2_feat')(x_esm)  # [D256] 64→256 (핵심 bottleneck 해소)
...
x = Dense(256, kernel_regularizer=regularizers.l2(1e-5), name='head_dense256')(concat)  # [D256] 64→256
x = Activation('relu')(x)
x = Dropout(0.2)(x)
...
print("[Model v8b-D256] Phase 2 LR: ESM2+head_dense256={} | CNN={} | pred_out={} | rest={}".format(
...
if IS_TYPE_B:
    ...
    proto_loss_fn = PrototypeContrastiveLoss(
        n_prototypes=PROTO_K, emb_dim=256,  # [D256]
...
print("\n[Final] GO={} | Type={} | k={} | Phase2 α={} | head_dense256 lr={}".format(
