# -*- coding: utf-8 -*-
# ============================================================================
# v6e_integrated_full_model.py  (v6d + adaptive Phase2 + no-LP)
#
# v6d 대비 핵심 변경:
# [v6e-1] Phase 0에서 GO term Type 자동 분류
#   pos_sep_ratio = inter_dist / intra_dist (Phase 0 ESM-2 임베딩 기준)
#   Type A (sep_ratio >= 1.15): positive 진화적 응집 → Phase 2 현행 설정 유지
#   Type B (sep_ratio < 1.15): positive 이질성 → Phase 2 제약 강화
#
# [v6e-2] Phase 2 Adaptive Hyperparameter
#   Type B: adam lr=0.0001 + clipnorm=0.5 + patience=3
#   Type A: adam lr=0.0003 + patience=7 (v6d 동일)
#   근거: Type B에서 Phase 2 CNN이 Phase 1 embedding 구조를 역전시킴
#         (sep_ratio: 1.37 -> 0.83, v6d GO:0006941 3-run 평균)
#
# [v6e-3] Phase 3 Label Propagation 제거 (alpha=0.0 고정)
#   근거: 5개 GO term 중 4개에서 LP가 AUPRC 감소 또는 중립
#         GO:0006096에서 LP 적용 시 AUPRC -8.9%
#
# [v6e-4] Coverage 정규화
#   clip(n_pos * 4.0 / 256, min=10, max=80) — positive당 균등 exposure
#   기존: clip(n_pos * 6.0 / 256, min=20, max=50) — Type B 불공정
#
# v6d 원본:
...
try:
    ...
    print("Usage: python v6e_integrated_full_model.py <GO_TERM>")
    sys.exit(1)
...
MARGIN_P1         = 0.3    # [I3]
TARGET_COVERAGE   = 4.0    # [v6e-4] 4.0 (v6d 6.0) — positive당 균등 exposure
...
# --------------------------------------------------------------------------
# [6-v6h] Phase 1 — GradientTape Triplet (4-modality, CNN frozen)
# --------------------------------------------------------------------------
...
def phase1_triplet_epoch_hybrid(
    ...
    """
    [v6d] Phase 1: ESM-2 + Domain + DeltaBranch 학습 (CNN frozen)
    CNN은 set_cnn_trainable(False) 상태여야 함.
    """
    ...
    for batch_i in range(n_batches):
        ...
        # CNN frozen → CNN 가중치에 gradient 없음 (자동 skip)
...
print("\n[Model v6e] ESM-2(640->64) + CNN(1500->32) + Domain(LSTM 16) + DomainDelta(251->16) -> concat[128]")
print("[Model v6e] Head: Dense(64) -> L2_norm -> emb[64] -> sigmoid")
print("[Model v6e] Phase 1: ESM-2 CNN(frozen) Domain DeltaBranch | Phase 2: ESM-2(frozen) CNN Domain DeltaBranch")
print("[Model v6e] v6e changes: adaptive Phase2 LR/patience | LP removed | coverage normalized")
...
if IS_TYPE_B:
    print("  -> Phase 2 will use: lr=0.0001, clipnorm=0.5, patience=3")
    ...
    print("  -> Phase 2 will use: lr=0.0003, patience=7 (standard)")
    print("  -> Biological note: positive set shares evolutionary origin — ESM-2 coherent")

# ==========================================================================
# PHASE 1: Triplet (ESM-2 + Domain 학습, CNN frozen)
# ==========================================================================
print("\n" + "=" * 55)
print(">>> PHASE 1: Triplet — ESM-2+Domain (CNN frozen)")
print("    margin={:.1f} [I3] | n_batches={} [I4] | max 15 epochs".format(
    MARGIN_P1, N_BATCHES_P1))
print("=" * 55)

# CNN frozen (Phase 1 동안 ESM-2 + Domain + DeltaBranch 학습)
set_cnn_trainable(feature_model, False)
print("  [Phase1-v6e] CNN frozen | ESM-2+Domain+DeltaBranch trainable")
...
for epoch in range(PHASE1_EPOCHS):
    ...
    if (epoch + 1) % 5 == 0:
        ...
        # [v6d-fix] margin_sat threshold early stop 제거 — fixed 15 epoch
        # 이유: threshold(60%)가 GO term마다 수렴 속도 차이로 인한 false stop 유발
        #       (GO:0003774: sat=61.6%@ep10 stop → AUPRC 0.4230 vs sat=70.7%@ep15 → 0.6794)
        #       active_rate < 2% × 4 epoch 조건만으로 수렴 판단 (이미 존재)
        print("  [Margin] sat={:.1f}% cdist={:.4f}".format(sat, cdist))
...
print("    [v6e-2] Adaptive hyperparameter by GO term type")
print("=" * 55)
...
if IS_TYPE_B:
    adam_p2       = optimizers.Adam(lr=0.0001, clipnorm=0.5)  # [v6e-2] 작은 업데이트
    NO_IMPROVE_LIMIT = 6   # [v6e-2fix] 6으로 수정 (3은 lr=0.0001의 느린 수렴에 너무 공격적)
    # 근거: v6e run1에서 patience=3이 epoch5에서 AUPRC=0.1511로 조기 종료
    #       v6d GO:0006941이 lr=0.0003으로 epoch1→9에서 0.17→0.28로 느리게 수렴
    #       lr=0.0001이면 더 느리므로 patience는 더 길어야 함
    print("  [v6e-2] Type-B: lr=0.0001, clipnorm=0.5, patience={}".format(NO_IMPROVE_LIMIT))
    print("  [v6e-2] Reason: CNN overfitting to sub-cluster sequence patterns")
else:
    adam_p2       = optimizers.Adam(lr=0.0003)  # v6d 동일
    NO_IMPROVE_LIMIT = 7   # v6d 동일
    print("  [v6e-2] Type-A: lr=0.0003, patience={} (standard)".format(NO_IMPROVE_LIMIT))
...
print("\n[Final] GO={} | Type={} | Phase2_LR={} | patience={}".format(
    ...
    '0.0001' if IS_TYPE_B else '0.0003',
    3 if IS_TYPE_B else 7))
