...
# [수정 1] 인자를 go_index(숫자)가 아닌 selected_go(문자열)로 바로 받음
...
# [수정 2] find_GO 함수는 사용하지 않으므로 주석 처리
# def find_GO(num):
#    fr = open('../data/raw_data/data/go_terms/go.txt')
#    for _ in range(num):
...
# [수정 3] find_GO 삭제 (selected_go 사용)
...
x1 = Dense(32, kernel_regularizer=regularizers.l2(0.00001))(x1) # 기존 0.15 -> 0.01로 변경
x1 = Activation('relu')(x1)
x1 = Dropout(0.2)(x1)
x1 = Dense(16, kernel_regularizer=regularizers.l2(0.00001))(x1) # 기존 0.15 -> 0.01로 변경
...
x = Dense(16, kernel_regularizer=regularizers.l2(0.00001))(x) # 기존 0.15 -> 0.01로 변경
x = Activation('relu')(x)
x = Dropout(0.2)(x) # 기존 0.5 -> 0.2로 변경
x = Dense(1, kernel_regularizer=regularizers.l2(0.00001))(x) # 기존 0.15 -> 0.01로 변경
...
adam = optimizers.Adam(lr = 0.001, beta_1 = 0.9, beta_2 = 0.999, epsilon = 1e-08, decay=0.0) # lr 기존 0.002 -> 0.001로 변경
...
model.save('../saved_models/' + safe_go + '_ensenbl_fitted_DNN.h5')
np.save('../saved_models/' + safe_go + '_ensenbl_fitted_CRF_weights.npy', theta)

# Write predictions
print "Writing Predictions to file..."
output_file = '../results/' + safe_go + '_ensenbl_fitted_scores.txt'
