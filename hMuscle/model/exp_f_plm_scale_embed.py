...
def parse_args():
    ...
    p.add_argument('--all-layers', action='store_true',
                    help='Extract every layer 1..n_layers (Fisher sweep) instead of just {L_mid, L_final}')
    return p.parse_args()
...
def main():
    ...
    if args.all_layers:
        print(f"  Layers: ALL 1..{L_final}  (Fisher sweep mode)")
    else:
