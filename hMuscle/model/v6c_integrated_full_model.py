...
#   Phase 1  : Triplet (ESM-2 + CNN + Domain) with gradient scaling
#              ESM-2: 0.2x | CNN: 1.0x
#   Phase 1.5: Linear Probing (all frozen, head only)
#   Phase 2  : Focal (ESM-2 frozen, CNN + Domain + head)
#              patience=7, alpha=0.25 (score collapse 방지)
#   Phase 3  : Label Propagation
#
# [ARCH] 모델 구조 (v6c-nogating)
...
#   Fusion       : direct concat[esm2_gated(64) + cnn_feat(32) + domain_feat(16) = 112]
#                  → Dense(48,relu) → L2_norm → emb[48] → Dense(1,sigmoid)
#   EMB_DIM=112 (v6h와 동일, Gating 제거로 Phase2 contamination 방지)
#
# [SEQ] 서열 입력
...
# [v6c-nogating] Gradient scaling constants (gate 제거로 단순화)
ESM2_GRAD_SCALE = 0.2
CNN_GRAD_SCALE  = 1.0
...
def phase1_triplet_epoch_hybrid(
    ...
    """
    [v6c] Phase 1: ESM-2 + CNN + Domain 학습 (gradient scaling, gate 제거)
    """
    ...
    for batch_i in range(n_batches):
        ...
        # [v6c-fix] ESM-2 frozen in Phase 1 → gradient scaling 불필요
        # NaN/Inf guard only
        grads_and_vars = []
        for g, v in zip(grads, feature_model.trainable_variables):
            ...
            g_safe = tf.where(tf.math.is_finite(g), g, tf.zeros_like(g))
            grads_and_vars.append((g_safe, v))
...
# [v6d-fix] Direct concat — Gating 제거 (Phase2 contamination 방지)
# esm2_gated[64] + cnn_feat[32] + domain_feat[16] = 112
concat = concatenate([esm2_gated, cnn_feat, domain_feat], name='feature_concat')
...
print("[Model v6c-nop1a] No Gating | No Phase1a | ESM-2+Domain Phase1 triplet | CNN Phase2")
...
# ==========================================================================
# [v6c-nop1a] Phase 1a 제거: CNN random init으로 Phase 1 진입
# 이유: Phase 1a CNN warmup이 triplet gradient를 희석시켜 margin_sat 저하
#       v6h(no Phase1a): margin_sat 60.5% vs v6c(Phase1a): 11.1%
# ==========================================================================
...
# [v6c-fix3] Phase 1: ESM-2 Dense + Domain LSTM trainable (v6h와 동등)
# 이유: Domain LSTM도 triplet으로 학습해야 의미있는 16-dim 표현 확립
# v6h: ESM-2+Domain 모두 Phase 1 trainable → 112-dim 중 80-dim이 의미있는 표현
# CNN/Gate만 Phase 2에서 학습 (잘 훈련된 ESM-2+Domain 위에서)
DOMAIN_LAYER_NAMES = {'dm_emb', 'domain_feat'}
for layer in feature_model.layers:
    layer.trainable = False
set_esm2_trainable(feature_model, True)   # esm2_d1, esm2_d2, esm2_feat
for layer in feature_model.layers:       # dm_emb, domain_feat
    if layer.name in DOMAIN_LAYER_NAMES:
        layer.trainable = True
print("  [Phase1-fix3] ESM-2 Dense + Domain LSTM trainable | CNN/Gate frozen")
...
# ==========================================================================
print("\n" + "=" * 55)
print(">>> PHASE 2: CNN Fine-tuning (No Gating)")
print("    [v6c] ESM-2 FROZEN | CNN+Domain TRAIN | Focal only")
...
print("  [Phase2] ESM-2 frozen | CNN + Domain trainable")
