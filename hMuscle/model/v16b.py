# -*- coding: utf-8 -*-
"""
v16b.py — Ablation: Gated Residual Fusion, Auxiliary Streams ALWAYS ZERO
==========================================================================

진단 목적: v16 대비 -0.26 AUPRC 퇴행 원인 분리
  원인 A: auxiliary 스트림(RNA/LOC/Splice)이 gene-level noise 주입
  원인 B: gate 아키텍처 자체가 ESM-2 gradient 흐름 교란

v16b 설계:
  - v16 동일 아키텍처 (Gated Residual Fusion 유지)
  - auxiliary 스트림 B/C/D: train + test 모두 zeros로 강제
  - use_bias=False 보장으로 h_aux_proj = 0 → h_fused = h_esm (항등)
  - gate params: h_aux_proj=0 → gradient=0 → 사실상 미학습 (forward pass 무영향)

기대 결과:
  v16b ≈ 0.7022 → 원인 A (auxiliary feature noise)
  v16b << 0.7022 → 원인 B (gate architecture disruption)

실행:
  cd /home/welcome1/sw1686/DIFFUSE/hMuscle/model
  conda activate isoform_env
  CUDA_VISIBLE_DEVICES=0 nohup python v16b.py > ../../logs_isoform/v16b_$(date +%Y%m%d_%H%M).log 2>&1 &
"""
...
OUT_DIR   = '../../reports/v16b'
os.makedirs(OUT_DIR, exist_ok=True)

N_SEEDS        = 5
BASELINE_AUPRC = 0.7022
FEAT_DROP_P    = 0.0    # v16b: auxiliary always zero → dropout 불필요
...
print("=" * 70)
print("  v16b — Ablation: Gated Fusion, ALL aux=ZERO (train+test)")
print(f"  Architecture: v16 (identical) | Streams B/C/D: forced zeros")
print(f"  Feature Dropout p={FEAT_DROP_P} | Baseline AUPRC={BASELINE_AUPRC}")
print(f"  Hypothesis: if v16b≈0.7022 → aux noise caused v16 regression")
print("=" * 70)
...
# v16b: DIFF_SPLICE 서브셋 구축용으로 원본 sd 보존 (zeroing 전)
X_te_sd_orig = X_te_sd.copy()

# v16b: auxiliary 스트림 B/C/D 완전 제로화 (train + test)
# → h_aux_proj = 0 → h_fused = h_esm → v15d architecture와 동등
# → gate params: dL/d_gate = dL/d_h_fused * h_aux_proj = 0 → no gradient
X_tr_sd[:]  = 0.0;  X_te_sd[:]  = 0.0
X_tr_rna[:] = 0.0;  X_te_rna[:] = 0.0
X_tr_loc[:] = 0.0;  X_te_loc[:] = 0.0
print("  [v16b] ALL auxiliary streams forced to ZERO (train + test)")
...
for g, idxs in multi_genes.items():
    embs   = X_te_esm[idxs]
    deltas = X_te_sd_orig[idxs]  # original (non-zeroed) for correct DS/NM split
...
# ── 9. 전체 GO term 루프 ─────────────────────────────────────────────────────
print(f"\n[3] Training v16b ablation ({N_SEEDS} seeds × {N_GO} GO terms)...")
...
print(f"\n{'='*70}")
print(f"  v16b ABLATION RESULTS  (elapsed: {elapsed/60:.1f} min)")
print(f"{'='*70}")
...
np.save(f'{OUT_DIR}/score_matrix_v16b_{ts}.npy', score_matrix)
...
with open(f'{OUT_DIR}/v16b_meta_{ts}.json', 'w') as f:
    json.dump(meta, f, indent=2)

print(f"\n  Score matrix → {OUT_DIR}/score_matrix_v16b_{ts}.npy")
print(f"  Meta         → {OUT_DIR}/v16b_meta_{ts}.json")
print(f"{'='*70}")
