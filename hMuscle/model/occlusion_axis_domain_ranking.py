...
rand_dir = rng.standard_normal(640).astype(np.float32); rand_dir /= np.linalg.norm(rand_dir)
conds = {f"axis{k}": [W[k]] for k in range(8)}
conds["random_dir"] = [rand_dir]
conds["all_8_axes"] = [W[k] for k in range(8)]
results = {"baseline": {"domain_ranking_auc": dr0, "n_pairs": npair, "macro_auprc": ap0}}
print(f"\n   {'condition':>12} {'DR-AUC':>8} {'ΔDR':>8} {'macroAUPRC':>11} {'Δmacro':>8}")
print(f"   {'baseline':>12} {dr0:>8.4f} {'—':>8} {ap0:>11.4f} {'—':>8}")
for name, dirs in conds.items():
    l15o = occlude(X_te_l15, LAYER_A, dirs)
    l30o = occlude(X_te_l30, LAYER_B, dirs)
    p = predict_ensemble(l15o, l30o)
    dr, _ = domain_ranking_auc(p); ap = macro_auprc(p)
    results[name] = {"domain_ranking_auc": dr, "delta_dr": dr0 - dr, "macro_auprc": ap, "delta_macro": ap0 - ap}
    print(f"   {name:>12} {dr:>8.4f} {dr0-dr:>+8.4f} {ap:>11.4f} {ap0-ap:>+8.4f}")

with open(f"{OUT}/occlusion_axis_domain_ranking.json", "w") as f:
    json.dump(results, f, indent=2)
log(f"[done] saved -> {OUT}/occlusion_axis_domain_ranking.json")
print("\n예측 검정: axis3 ΔDR 최대(domain 축 인과 사용) & axis5(length) ΔDR≈random(랭킹에 안 씀)이면 인과 확정.")
