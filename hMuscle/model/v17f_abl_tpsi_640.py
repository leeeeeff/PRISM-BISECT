...
# -*- coding: utf-8 -*-
"""
v17f_abl_tpsi_640.py
--------------------
Ablation: T_psi with output dim = 640 (same as ESM2 L30).

Completes the delta_share isolation series:
  delta_share = T_psi_dim / (T_psi_dim + 640)

  v17f         : T_psi(640->64)   + triplet -> 9.1%  -> AUPRC 0.717
  tpsi_256     : T_psi(640->256)  + triplet -> 28.6% -> AUPRC 0.722
  THIS         : T_psi(640->640)  + triplet -> 50.0% -> AUPRC ?
  v17f*        : no T_psi         + no trip -> 50.0% -> AUPRC 0.734

At identical delta_share (50%), THIS vs v17f* isolates the pure triplet effect:
  THIS > v17f* : triplet organization adds value over raw delta_layer
  THIS ~ v17f* : triplet is neutral; dense transform + L2-norm ~ MaxAbsScaler
  THIS < v17f* : triplet + dense transform hurts (info loss from nonlinear map)
"""
...
OUT_DIR   = '../../reports/v17f_abl_tpsi_640'
...
BATCH_MLP    = 512
EPOCHS_MLP   = 60
EMBED_DIM_T  = 640          # same dim as L30 → delta_share = 50%
MAX_GO_TRIPS = 30000
GO_TRIPS_PER = 300
LAYER_A      = 15
LAYER_B      = 30

delta_share = EMBED_DIM_T / (EMBED_DIM_T + 640)
...
print("=" * 65)
print(f"  v17f Ablation: T_psi(640->{EMBED_DIM_T}) — pure triplet effect")
print(f"  delta_share = {EMBED_DIM_T}/{EMBED_DIM_T+640} = {delta_share:.1%} (same as v17f*)")
print(f"  Control: v17f*(no triplet, same 50%) = 0.7343")
print("=" * 65)
...
def build_T_psi_640(delta_dim=640, embed_dim=640):
    ...
    return models.Model(inp, out, name='T_psi_640')

def build_mlp_stage2(t_dim=640, esm_dim=640, n_go=82):
    inp_t   = layers.Input(shape=(t_dim,))
    inp_esm = layers.Input(shape=(esm_dim,))
    x = layers.Concatenate()([inp_t, inp_esm])    # 1280-dim — same as v17f*
    x = layers.Dense(256, activation='relu')(x)
    x = layers.BatchNormalization()(x)
    x = layers.Dropout(0.2)(x)
    x = layers.Dense(128, activation='relu')(x)
    out = layers.Dense(n_go, activation='sigmoid')(x)
    return models.Model([inp_t, inp_esm], out, name='MLP_640')
...
print(f"\n[6] Stage 1: T_psi(640->640) triplet training ({EPOCHS_T} epochs)...")
t0 = time.time()
tf.random.set_seed(42)
T_psi    = build_T_psi_640()
...
T256_ALL  = 0.7223; T256_L2   = 0.6251
VSTAR_ALL = 0.7343; VSTAR_L2  = 0.6366   # bootstrap point estimate
PRISM_ALL = 0.5962; PRISM_L2  = 0.3501

# At identical delta_share (50%), compare THIS vs v17f*
triplet_delta = auprc_all - VSTAR_ALL

if auprc_all > VSTAR_ALL + 0.005:
    verdict = f"Triplet HELPS at 50% delta_share: +{triplet_delta:+.4f} vs v17f*. Use T_psi(640) as final model."
elif abs(auprc_all - VSTAR_ALL) <= 0.005:
    verdict = f"Triplet NEUTRAL: THIS~v17f* ({triplet_delta:+.4f}). Raw delta_layer == T_psi-organized."
else:
    verdict = f"Triplet HURTS at 50%: {triplet_delta:+.4f} vs v17f*. Dense transform+L2norm adds noise. v17f* is final."

print(f"\n{'='*65}")
print(f"  Complete delta_share ablation series")
print(f"{'='*65}")
print(f"  Model            AUPRC   L2_Struct  delta%  triplet")
print(f"  --------------------------------------------------------")
print(f"  PRISM            {PRISM_ALL:.4f}  {PRISM_L2:.4f}     —      no")
print(f"  v17f (T_psi->64) {V17F_ALL:.4f}  {V17F_L2:.4f}   9.1%    yes")
print(f"  T_psi->256       {T256_ALL:.4f}  {T256_L2:.4f}  28.6%    yes")
print(f"  T_psi->640(THIS) {auprc_all:.4f}  {auprc_l2:.4f}  50.0%    yes")
print(f"  v17f* (no T_psi) {VSTAR_ALL:.4f}  {VSTAR_L2:.4f}  50.0%    no")
print(f"")
print(f"  Triplet effect at equal delta_share: {triplet_delta:+.4f}")
print(f"  Verdict: {verdict}")
print(f"{'='*65}")

results = {
    'architecture': {
        'method': 'T_psi_640_v17f_data',
        'T_psi': '640->512(ReLU)->BN->Drop(0.3)->640(ReLU)->L2norm',
        'stage2_input': 'concat[T_psi(640), L30(640)] = 1280',
        'delta_share_pct': 50.0,
    ...
    'triplet_effect_at_50pct': triplet_delta,
    'references': {
        'prism':     {'all_mf': PRISM_ALL, 'l2': PRISM_L2, 'delta_share_pct': 0},
        'v17f_64':   {'all_mf': V17F_ALL,  'l2': V17F_L2,  'delta_share_pct': 9.1},
        't_psi_256': {'all_mf': T256_ALL,  'l2': T256_L2,  'delta_share_pct': 28.6},
