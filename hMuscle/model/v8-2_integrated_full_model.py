# -*- coding: utf-8 -*-
# ============================================================================
# v8-1_integrated_full_model.py  (v8b + Bidirectional FiLM)
#
# v8b 결과 (2026-04-30):
#   Macro-AUPRC 0.315 → 0.352 (+11.7%) — v8b PASS
#   GO:0007204 AUPRC=0.1462, GO:0006941=0.1177(+38.5%), GO:0003774=0.5686(+35.4%)
#   [F5] 해결: Type-B centroid_cos Δ < 0.003 (geometry 보존 확인)
#
# v8-1 추가 변경: Bidirectional FiLM Block [v8-1]
#
# 근거 (embedding 분석에서 확인):
#   GO:0007204 Group 1 (134/310 pos, 43%): sep_ratio=0.650 — neg와 혼합
#   GO:0030017 Group 2 (219/452 pos, 48%): sep_ratio=0.691 — neg와 혼합
#   단순 concatenation으로는 ESM-2↔CNN 상호보완 불가
#   → FiLM: 전역 맥락(ESM-2)이 국소 모티프(CNN) 중요도 결정 [Weatheritt 2012]
#
# [v8-1] Bidirectional FiLM Block
#   방향 1 (ESM-2 → CNN): cnn_mod = gamma_c * cnn_feat + beta_c
#     gamma_c = Dense(32, init='zeros', bias='ones')(esm2_gated)
#     beta_c  = Dense(32, init='zeros', bias='zeros')(esm2_gated)
#   방향 2 (CNN → ESM-2): esm2_mod = gamma_e * esm2_gated + beta_e
#     sg_cnn  = tf.stop_gradient(cnn_feat)  → CNN grad → ESM-2 weights 차단 [F2]
#     gamma_e = Dense(64, init='zeros', bias='ones')(sg_cnn)
#     beta_e  = Dense(64, init='zeros', bias='zeros')(sg_cnn)
#   Residual fusion:
#     delta   = Dense(64, activation='tanh')([esm2_mod, cnn_mod])  ← concat 96-dim input
#     z_fused = esm2_gated + 0.1 * delta  ← residual, scale=0.1로 초기 안정성 확보
#
# [ARCH] v8-1
#   ESM-2: 640→64 (esm2_gated[64])
#   CNN:   1500→32 (cnn_feat[32])
#   FiLM:  residual_film_fusion(esm2_gated[64], cnn_feat[32]) → z_fused[64]
#   Fusion: concat([z_fused(64), domain_feat(16), dd_feat(16)]) = 96-dim  ← 128→96
#   Head:   Dense(64) → L2_norm → emb[64] → Dense(1,sigmoid)
#
# [v8-1] Discriminative LR (6그룹, FiLM 추가)
#   ESM-2 + head_dense64: lr=1e-5 (geometry 보호)
#   CNN:                  lr=1e-4
#   FiLM (film_*):        lr=5e-4 (modulation 빠르게 학습)
#   Domain/DomainDelta:   lr=1e-4
#   prediction_out:       lr=1e-3
#
# Phase 1: ESM-2+Domain trainable, CNN frozen, FiLM frozen (curriculum 보존)
# Phase 2: Unified Loss α=0.2 + 6그룹 discriminative LR (FiLM 추가)
#
# 검증 기준: GO:0007204 AUPRC ≥ 0.160 (v8b 0.1462 대비 +9.4%)
#           Macro-AUPRC ≥ 0.370 (v8b 0.352 대비 +5%)
# ============================================================================
import numpy as np
import sys
import os
...
try:
    ...
    print("Usage: python v8-1_integrated_full_model.py <GO_TERM>")
    sys.exit(1)
...
# [v8b-1] Phase 2 discriminative LR (v8-1: FiLM 그룹 추가)
LR_P2_SLOW = 1e-5   # ESM-2 + head_dense64 (geometry 보호)
LR_P2_CNN  = 1e-4   # CNN branch
LR_P2_FILM = 1e-4   # [v8-2] FiLM layers (film_delta 안정화: 5e-4→1e-4)
...
# [v8-1] FiLM residual scale
FILM_RESIDUAL_SCALE = 0.05  # [v8-2] 0.1→0.05: identity init 후 더 보수적 residual

# Layer name groups
ESM2_TRAINABLE_NAMES  = {'esm2_d1', 'esm2_d2', 'esm2_feat'}
CNN_TRAINABLE_NAMES   = {'cnn_emb', 'cnn_conv1', 'cnn_conv2', 'cnn_feat'}
FILM_LAYER_NAMES      = {'film_gamma_c', 'film_beta_c',        # [v8-1] ESM→CNN
                         'film_gamma_e', 'film_beta_e',        # [v8-1] CNN→ESM
                         'film_delta'}                         # [v8-1] residual fusion
...
def set_film_trainable(model, trainable):
    """[v8-1] Phase 1에서 FiLM frozen (CNN random init 오염 방지 [F2])"""
    for layer in model.layers:
        if layer.name in FILM_LAYER_NAMES:
            layer.trainable = trainable
...
def build_var_groups_p2(model):
    ...
    for layer in model.layers:
        ...
        if layer.name in ESM2_TRAINABLE_NAMES or layer.name in HEAD_DENSE_NAMES:
            ...
            cnn_ids |= ids
        elif layer.name in FILM_LAYER_NAMES:
            film_ids |= ids
        elif layer.name in PRED_OUT_NAMES:
            pred_ids |= ids
    return slow_ids, cnn_ids, film_ids, pred_ids

def apply_grads_p2(grads, variables, slow_ids, cnn_ids, film_ids, pred_ids,
                   opt_slow, opt_cnn, opt_film, opt_rest, opt_pred):
    """[v8-1] 6그룹 gradient dispatch"""
    g_slow, g_cnn, g_film, g_pred, g_rest = [], [], [], [], []
    ...
    if g_film: opt_film.apply_gradients(g_film)
...
def phase2_unified_epoch_v8b(
        ...
        slow_ids, cnn_ids, film_ids, pred_ids,
        opt_slow, opt_cnn, opt_film, opt_rest, opt_pred,
    ...
    for batch_i in range(n_batches):
        ...
        if is_type_b:
            ...
            apply_grads_p2(grads, all_vars, slow_ids, cnn_ids, film_ids, pred_ids,
                           opt_slow, opt_cnn, opt_film, opt_rest, opt_pred)
...
# [9] 데이터 로딩 (v7c 동일)
# --------------------------------------------------------------------------
print('>>> Preparing Data (v8-1) for ' + selected_go)
...
# ── [v8-1] Bidirectional FiLM Block ─────────────────────────────────────────
# 방향 1: ESM-2 → CNN (전역 맥락이 국소 모티프 중요도 결정)
# identity init: gamma=1(bias_ones), beta=0(bias_zeros), kernel='zeros'
film_gamma_c = Dense(32, kernel_initializer='zeros', bias_initializer='ones',
                     name='film_gamma_c')(esm2_gated)   # [64] → [32]
film_beta_c  = Dense(32, kernel_initializer='zeros', bias_initializer='zeros',
                     name='film_beta_c')(esm2_gated)    # [64] → [32]
cnn_modulated = Lambda(lambda x: x[0] * x[1] + x[2],
                        name='film_cnn_mod')([film_gamma_c, cnn_feat, film_beta_c])

# 방향 2: CNN → ESM-2 (stop_gradient: CNN grad → ESM-2 projection 차단 [F2])
sg_cnn = Lambda(lambda x: tf.stop_gradient(x), name='film_sg_cnn')(cnn_feat)
film_gamma_e = Dense(64, kernel_initializer='zeros', bias_initializer='ones',
                     name='film_gamma_e')(sg_cnn)       # [32] → [64]
film_beta_e  = Dense(64, kernel_initializer='zeros', bias_initializer='zeros',
                     name='film_beta_e')(sg_cnn)        # [32] → [64]
esm2_modulated = Lambda(lambda x: x[0] * x[1] + x[2],
                         name='film_esm2_mod')([film_gamma_e, esm2_gated, film_beta_e])

# Residual fusion: z_fused = esm2_gated + 0.1 * tanh(Dense(64)([esm2_mod, cnn_mod]))
film_concat = concatenate([esm2_modulated, cnn_modulated], name='film_concat')  # 64+32=96
film_delta  = Dense(64, activation='tanh',
                    kernel_initializer='zeros',  # [v8-2] identity init: z_fused≈esm2_gated at t=0
                    kernel_regularizer=regularizers.l2(1e-5),
                    name='film_delta')(film_concat)
# scale=0.1: 초기 학습에서 identity에 가깝게 유지 (CNN random init 오염 방지)
z_fused = Lambda(lambda x: x[0] + FILM_RESIDUAL_SCALE * x[1],
                  name='film_fused')([esm2_gated, film_delta])  # [64]
# ─────────────────────────────────────────────────────────────────────────────

# feature_model 출력: [z_fused(64), domain(16), dd(16)] = 96-dim (v8b 128→96)
concat = concatenate([z_fused, domain_feat, dd_feat], name='feature_concat')
...
print("\n[Model v8-1] Architecture: v8b + Bidirectional FiLM")
print("[Model v8-1] FiLM: ESM2[64]↔CNN[32] bidirectional, residual scale={}".format(FILM_RESIDUAL_SCALE))
print("[Model v8-1] Feature dim: 128 → 96 (z_fused[64]+domain[16]+dd[16])")
print("[Model v8-1] Phase 1: ESM-2+Domain trainable | CNN+FiLM frozen")
print("[Model v8-1] Phase 2: Unified Loss α={} + 6-group discriminative LR".format(ALPHA_P2))
print("[Model v8-1] Phase 2 LR: ESM2+head={} | CNN={} | FiLM={} | pred={} | rest={}".format(
    LR_P2_SLOW, LR_P2_CNN, LR_P2_FILM, LR_P2_PRED, LR_P2_REST))
...
set_film_trainable(feature_model, False)   # [v8-1] FiLM frozen (CNN random init 오염 방지)
print("  [Phase1] CNN frozen | FiLM frozen | ESM-2 + Domain + DeltaBranch trainable")
...
# [v8-1] Phase 2 discriminative optimizers (6그룹, FiLM 추가)
...
opt_p2_film = optimizers.Adam(lr=LR_P2_FILM, clipnorm=1.0)  # FiLM layers
...
slow_ids, cnn_ids_p2, film_ids_p2, pred_ids = build_var_groups_p2(base_model)
n_rest = len(classification_model.trainable_variables) - len(slow_ids) - len(cnn_ids_p2) - len(film_ids_p2) - len(pred_ids)
print("  [Phase2] Var groups: slow={} CNN={} FiLM={} pred={} rest={}".format(
    len(slow_ids), len(cnn_ids_p2), len(film_ids_p2), len(pred_ids), n_rest))

if IS_TYPE_B:
    # v8b에서 Type-B 25 epoch에서도 단조 증가 확인 (patience 한 번도 미소진)
    # → 40 epoch, patience 15로 수렴 여유 확보
    PHASE2_MAX_EPOCHS = 40
    NO_IMPROVE_LIMIT  = 15
else:
...
for epoch in range(PHASE2_MAX_EPOCHS):
    ...
    avg_m, avg_f, active_rate = phase2_unified_epoch_v8b(
        ...
        X_seq=X_train_seq_upsmp, X_dm=X_train_dm_upsmp,
        X_dd=X_train_dd_upsmp, y=y_train_upsmp,
        slow_ids=slow_ids, cnn_ids=cnn_ids_p2,
        film_ids=film_ids_p2, pred_ids=pred_ids,
        opt_slow=opt_p2_slow, opt_cnn=opt_p2_cnn,
        opt_film=opt_p2_film, opt_rest=opt_p2_rest, opt_pred=opt_p2_pred,
...
print("\n[Final] GO={} | Type={} | k={} | Phase2 α={} | FiLM lr={} | head lr={}".format(
    selected_go, go_type_str, PROTO_K if IS_TYPE_B else 'N/A',
    ALPHA_P2, LR_P2_FILM, LR_P2_SLOW))
