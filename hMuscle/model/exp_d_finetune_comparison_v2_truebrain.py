...
def run_finetune_experiment(n_unfrozen_layers, exp_label):
    ...
    # D2 OOM fix: after 30-layer FT, Adam moment states (~1.2GB for 147M params) +
    # per-parameter .grad buffers stay resident and fragment the 24GB card, OOMing the
    # (correctly no_grad) extraction below. Free them before switching to inference.
    del optimizer
    ...
    # Extract L30 embeddings from fine-tuned model (smaller batch for the 30-layer case
    # to keep the extraction peak well under budget on a shared 24GB card).
    _emb_bs = 8 if n_unfrozen_layers == 30 else BATCH_EMB
    print(f"  Extracting fine-tuned L30 for training set ({len(tr_seq_list)} isoforms, bs={_emb_bs})...")
    X_tr_ft = extract_l30_embeddings(ft_model, ft_batch_converter, tr_seq_list, batch_size=_emb_bs)
    print(f"  Extracting fine-tuned L30 for brain test set ({len(te_seq_list)} isoforms, bs={_emb_bs})...")
    X_te_ft = extract_l30_embeddings(ft_model, ft_batch_converter, te_seq_list, batch_size=_emb_bs)
...
# Resume: a prior run already completed D1's 160-min fine-tune and saved its L30
# embeddings. Reuse them (re-evaluate only, seconds) instead of re-fine-tuning.
_d1_tr = f'{OUT_DIR}/ft_D1_train_l30.npy'
_d1_te = f'{OUT_DIR}/ft_D1_test_l30.npy'
if os.path.exists(_d1_tr) and os.path.exists(_d1_te):
    print("\n[D1] Reusing saved fine-tuned embeddings (skip re-fine-tune)...")
    X_tr_d1 = np.load(_d1_tr).astype(np.float32)
    X_te_d1 = np.load(_d1_te).astype(np.float32)
    aps_d1, _ = train_and_eval_prism(X_tr_d1, Y_tr, X_te_d1, Y_te)
    all_d1, l2_d1 = report_auprc(aps_d1, 'D1: Fine-tune 5 layers (resumed)')
else:
