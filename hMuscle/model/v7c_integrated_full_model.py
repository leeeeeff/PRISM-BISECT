# -*- coding: utf-8 -*-
# ============================================================================
# v7c_integrated_full_model.py  (v7b + Prototype-anchored InfoNCE with Negatives)
#
# v7c 핵심 변경 (v7b 대비):
# [v7c-1] Phase 1 Type-B — Proto-kN + Negative InfoNCE:
#   기존 분모: Σ_j exp(sim(x_i, proto_j)/τ)              ← prototype만
#   v7c 분모: Σ_j exp(sim(x_i, proto_j)/τ)
#            + Σ_{n∈neg_batch} exp(sim(x_i, x_n)/τ)       ← negative 포함
#   GradientTape 안에서 pos + neg 모두 feature_model 실행
#   → gradient: pos는 prototype 방향으로 당김, neg는 prototype 공간에서 밀어냄
#   근거: v7b에서 Centroid dist 0.106→0.011로 급락 (negative가 prototype 공간으로 drift).
#         negative를 분모에 포함하면 별도 λ 없이 repulsion이 자연스럽게 작동.
#
# [v7c-2] Prototype Biological Validation:
#   Phase 1 완료 후 prototype 할당(assignments) 저장
#   → UniProt/InterPro 어노테이션 교차 검증용
#
# v7b 변경사항 유지:
# [v7b-1] Phase 1 Type-A=Triplet | Type-B=Proto-kN (Gap Statistic)
...
VER_TAG  = "v7c_noesm2_integrated" if os.environ.get('NO_ESM2', '0') == '1' else "v7c_integrated"
...
# [DIAG-A] ESM-2 비활성화 플래그 — 환경변수 NO_ESM2=1 로 제어
if os.environ.get('NO_ESM2', '0') == '1':
    print("[DIAG-A] NO_ESM2=1 — ESM-2 mask를 0으로 고정 (ESM-2 branch 완전 비활성화)")
    X_test_esm2_mask       = np.zeros_like(X_test_esm2_mask)
    X_train_esm2_mask      = np.zeros_like(X_train_esm2_mask)
    X_train_other_esm2_mask = np.zeros_like(X_train_other_esm2_mask)
...
print("\n" + "=" * 58)
if IS_TYPE_B:
    print(">>> PHASE 1: Proto-kN + Neg-InfoNCE [v7c-1] — ESM-2+Domain (CNN frozen)")
    print("    k={} | τ={} | λ_div={} | EMA α={} | neg_in_denom=True".format(
...
# [v7c-2] Prototype Biological Validation — Phase 1 완료 후 할당 저장
if IS_TYPE_B and proto_loss_fn is not None:
    save_prototype_assignments(
        proto_loss_fn,
        emb_by_phase['Ph1(contrastive)'],
        y_test,
        save_dir=SAVE_DIR,
        ver_tag=VER_TAG,
        go_term=selected_go)
...
print("\n[Final] GO={} | Type={} | Phase1={} | k={} | WarmInit={} | NegInfoNCE={}".format(
    selected_go, go_type_str,
    'Proto-kN+NegInfoNCE' if IS_TYPE_B else 'Triplet',
    PROTO_K if IS_TYPE_B else 1,
    'OK' if warm_ok else 'FAIL',
    'ON' if IS_TYPE_B else 'N/A'))
