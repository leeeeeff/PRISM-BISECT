...
# -*- coding: utf-8 -*-
"""
v17f_abl_no_tpsi_v2_truebrain.py
---------------------------------
TISSUE-MISLABELING BUGFIX RERUN (2026-07-14).
Original v17f_abl_no_tpsi.py loaded my_gene_list_fixed.npy / esm2_layer_{NN}_t30_150M.npy
as its "test" set -- these are MUSCLE data (BambuTx/NM_ IDs, 36748 isoforms),
despite being described in the manuscript as "brain zero-shot". This script
re-points the TEST side only at the TRUE brain isoform set
(hMuscle/data/brain_isoquant_esm2/full/brain_full_*, IsoQuant IDs like
A1BG-204, 63994 isoforms total / 18514 unique genes). Training side
(train_gene_list.npy etc.) is UNCHANGED.

Original (mislabeled-as-brain, actually muscle): v17f_abl_no_tpsi.py, backed up
at v17f_abl_no_tpsi_backup_20260714.py before this rerun was created.
...
OUT_DIR    = '../../reports/truebrain_rerun_20260714/v17f_abl_no_tpsi'
...
X_te_l30 = np.load(f'{BRAIN_DIR}/brain_full_esm2_layer{LAYER_B:02d}_t30_150M.npy').astype(np.float32)
X_te_l15 = np.load(f'{BRAIN_DIR}/brain_full_esm2_layer{LAYER_A:02d}_t30_150M.npy').astype(np.float32)
...
print(f"  Train: {len(tr_genes)}  Test (TRUE BRAIN): {len(te_sym_list)}")
