...
# -*- coding: utf-8 -*-
"""
run_GPU_v7c.py — v7c 2-GPU 병렬 실행 스크립트

v7c 변경 (v7b 대비):
  [v7c-1] Phase 1 Type-B: Proto-kN + Negative InfoNCE
           분모에 negative embeddings 포함 → centroid drift 방지
  [v7c-2] Phase 1 완료 후 prototype assignments 저장 (생물학적 검증용)

acceptance criterion:
  Macro-AUPRC > 0.485 (v6g 0.465 + 0.02)
  GO:0006941 AUPRC > 0.1209 (v6g 해당 term baseline)
  Phase 1 Centroid dist: Phase 0 대비 유지 또는 향상

실행:
  cd hMuscle/model/
  conda activate isoform_env
  python run_GPU_v7c.py
"""
...
MODEL_SCRIPT = "v7c_integrated_full_model.py"
...
def run_model_on_gpu(go_terms, gpu_id):
    ...
    print("=" * 60)
    print(" [GPU {}] v7c — {} GO terms".format(gpu_id, total))
    print("=" * 60)
    ...
    for i, go_term in enumerate(go_terms):
        ...
        log_file = os.path.join(log_folder,
                                'v7c_{}_{}_Full.log'.format(safe_go, date_str))
...
if __name__ == '__main__':
    print("=" * 60)
    print(" v7c Runner")
    print(" [v7c-1] Phase 1 Type-B: Proto-kN + Negative InfoNCE")
    print(" [v7c-2] Prototype Biological Validation (assignments saved)")
    print(" acceptance: Macro-AUPRC > 0.485 | GO:0006941 > 0.1209")
    print("=" * 60)
    ...
    print(" Results: hMuscle/results_isoform/GO_*/v7c_integrated_*/")
    print(" Logs:    {}GO_*/v7c_*_Full.log".format(LOG_DIR))
    print("=" * 60)
