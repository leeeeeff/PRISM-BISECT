# -*- coding: utf-8 -*-
# ============================================================================
# v6f_integrated_full_model.py  (v6e + lr/patience/coverage fix)
#
# v6e 대비 핵심 변경:
# [v6f-1] Type-B Phase 2 LR 수정: 0.0001 → 0.0002
#   근거: v6e에서 GO:0006941 Phase 2 epoch1 AUPRC=0.150에서 lr=0.0001로
#         15 epoch 내 v6d 수준(0.284) 도달 불가. GO:0007204는 0.0001로도 수렴.
#         0.0002 = 수렴 속도 확보 + 과적합 방지 균형
#
# [v6f-2] Type-B patience: 6 → 10, max_epochs: 15 → 25
#   근거: lr=0.0002에서 epoch당 AUPRC 개선 ~0.004 예상 → best 도달 ~20 epoch
#
# [v6f-3] Coverage 원복: TARGET_COVERAGE 4 → 6, clip_min 10 유지
#   근거: v6e에서 GO:0006096 coverage 50% 감소 → 불필요한 성능 하락
#         min=10 유지 (v6e에서 올바르게 개선됨), max=80 유지
#
# [v6f-4] [Final] 출력 patience 하드코딩 버그 수정
#   기존: `patience=3 if IS_TYPE_B else 7` (하드코딩)
#   수정: `NO_IMPROVE_LIMIT` 변수 사용
#
# v6e 원본 변경사항 (유지):
...
try:
    ...
    print("Usage: python v6f_integrated_full_model.py <GO_TERM>")
    sys.exit(1)
...
print("\n[Model v6f] ESM-2(640->64) + CNN(1500->32) + Domain(LSTM 16) + DomainDelta(251->16) -> concat[128]")
print("[Model v6f] Head: Dense(64) -> L2_norm -> emb[64] -> sigmoid")
print("[Model v6f] Phase 1: ESM-2 CNN(frozen) Domain DeltaBranch | Phase 2: ESM-2(frozen) CNN Domain DeltaBranch")
print("[Model v6f] v6f changes: TypeB lr=0.0002(0.0001), patience=10(6), max_epochs=25(15), coverage=6(4)")
...
print("    [v6f] Adaptive hyperparameter by GO term type")
print("=" * 55)
...
if IS_TYPE_B:
    ...
    print("  [v6f] Type-A: lr=0.0003, patience={} (standard)".format(NO_IMPROVE_LIMIT))

classification_model.compile(
    loss=binary_focal_loss(gamma=2.0, alpha=0.10),
    optimizer=adam_p2, metrics=['accuracy'])

PHASE2_MAX_EPOCHS  = 25 if IS_TYPE_B else 15  # [v6f-2] Type-B 25 epoch (v6e 15)
BATCH_SIZE_P2      = 512
