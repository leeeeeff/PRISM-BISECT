# -*- coding: utf-8 -*-
"""
v16c.py — 4-Stream Gated Residual Fusion (Random State Fix)
=============================================================

v16 원인 진단 결과:
  v16b (zero aux) ≈ v16 (real aux) ≈ 0.44 << v15d 0.7022
  → auxiliary 스트림 content가 아닌 **TF random state shift** 문제

근본 원인:
  v16에서 extra Dense(enc_splice/rna/loc, aux_proj, gate)가
  global TF random state에서 ~46,432개 값을 소비 →
  head Dense(64)/Dense(1)가 v15d와 완전히 다른 초기화를 얻음
  → 다른 local minimum → -0.26 AUPRC 퇴행

v16c 핵심 수정:
  auxiliary 레이어들에 operation-level fixed seed 적용
  → global random state 소비 없음
  → ESM-2 path + head Dense(64)/Dense(1)가 v15d와 동일한 초기화
  → fallback(zero aux) 시 v15d 성능 복원 + aux 기여 가산

AUX_INIT_SEED = 2000 (v15d와 충돌하지 않는 별도 seed 공간)

실행:
  cd /home/welcome1/sw1686/DIFFUSE/hMuscle/model
  conda activate isoform_env
  CUDA_VISIBLE_DEVICES=1 nohup python v16c.py > ../../logs_isoform/v16c_$(date +%Y%m%d_%H%M).log 2>&1 &
"""
...
OUT_DIR   = '../../reports/v16c'
os.makedirs(OUT_DIR, exist_ok=True)

N_SEEDS        = 5
BASELINE_AUPRC = 0.7022
FEAT_DROP_P    = 0.20   # Feature Dropout: 각 auxiliary stream 비활성화 확률
AUX_INIT_SEED  = 2000   # fixed seed for aux layers → no global random state consumption
...
print("=" * 70)
print("  v16c — 4-Stream (Random State Fix) Gated Residual Fusion")
...
def build_model(esm_dim, sd_dim, rna_dim, loc_dim):
    ...
    # ── Stream B: Splice delta ─────────────────────────────────────────────
    # v16c FIX: operation-level fixed seeds → does NOT consume global TF random state
    # → ESM-2 path + head Dense(64)/Dense(1) get SAME initialization as v15d
    _gi = tf.keras.initializers.GlorotUniform

    sd_drop = StreamDropout(p=FEAT_DROP_P, name='drop_splice')(inp_sd)
    h_splice = layers.Dense(32, activation='relu', use_bias=False, name='enc_splice',
                            kernel_initializer=_gi(seed=AUX_INIT_SEED))(sd_drop)  # (32,)
    h_splice = layers.Dropout(0.1)(h_splice)

    # ── Stream C: RNA stability ────────────────────────────────────────────
    rna_drop = StreamDropout(p=FEAT_DROP_P, name='drop_rna')(inp_rna)
    h_rna = layers.Dense(32, activation='relu', use_bias=False, name='enc_rna',
                         kernel_initializer=_gi(seed=AUX_INIT_SEED+1))(rna_drop)  # (32,)
    h_rna = layers.Dropout(0.1)(h_rna)

    # ── Stream D: Subcellular localization ────────────────────────────────
    loc_drop = StreamDropout(p=FEAT_DROP_P, name='drop_loc')(inp_loc)
    h_loc = layers.Dense(32, activation='relu', use_bias=False, name='enc_loc',
                         kernel_initializer=_gi(seed=AUX_INIT_SEED+2))(loc_drop)  # (32,)
    h_loc = layers.Dropout(0.1)(h_loc)

    # ── Auxiliary aggregation ─────────────────────────────────────────────
    h_aux      = layers.Concatenate(name='aux_concat')([h_splice, h_rna, h_loc])  # (96,)
    h_aux_proj = layers.Dense(128, use_bias=False, name='aux_proj',
                              kernel_initializer=_gi(seed=AUX_INIT_SEED+3))(h_aux)  # (128,)

    # ── Unified gate ──────────────────────────────────────────────────────
    gate_in = layers.Concatenate()([h_esm, h_aux])                       # (224,)
    gate    = layers.Dense(128, activation='sigmoid', name='gate',
                           kernel_initializer=_gi(seed=AUX_INIT_SEED+4),
                           bias_initializer='zeros')(gate_in)             # (128,)
...
print(f"\n{'='*70}")
print(f"  v16c RESULTS  (elapsed: {elapsed/60:.1f} min)")
print(f"{'='*70}")
...
np.save(f'{OUT_DIR}/score_matrix_v16c_{ts}.npy', score_matrix)

meta = {
    'model': 'v16c',
    'aux_init_seed': AUX_INIT_SEED,
    'timestamp': ts,
...
with open(f'{OUT_DIR}/v16c_meta_{ts}.json', 'w') as f:
    json.dump(meta, f, indent=2)

print(f"\n  Score matrix → {OUT_DIR}/score_matrix_v16c_{ts}.npy")
print(f"  Meta         → {OUT_DIR}/v16c_meta_{ts}.json")
print(f"{'='*70}")
