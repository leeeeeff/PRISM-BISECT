...
All AUPRC values computed on the same 82 brain MF GO term test set
(36,748 brain isoforms, gene2go annotations, ≥2 positives per term).
"""
...
GOA_FILE    = f'{SPROT_DIR}/goa_uniprot_sprot.gaf.gz'
...
# Extract brain test protein sequences in ESM-2 embedding order
if not os.path.exists(QUERY_FASTA):
    print("  Extracting brain test protein sequences...")
    PEP_FILE = f'{DATA_DIR}/brain_esm2/brain_only_transcripts.fa.transdecoder.pep'

    # Build index: ENST_id → sequence
    pep_index = {}
    ...
    with open(PEP_FILE) as f:
        for line in f:
            ...
            if line.startswith('>'):
                ...
                # Header: >ENST00000003084.p1 GENE....
                raw_id = line[1:].split()[0]
                # Try full id then strip version
                current_id = raw_id
                current_seq = []
            else:
    ...
    # Also index by base ENST (without .p suffix and version)
    pep_by_enst = {}
    for pid, seq in pep_index.items():
        base = pid.split('.')[0]  # ENST00000003084
        if base not in pep_by_enst:
            pep_by_enst[base] = (pid, seq)

    print(f"  Protein sequences indexed: {len(pep_index)} entries, {len(pep_by_enst)} unique ENSTs")

    written, missing = 0, 0
    with open(QUERY_FASTA, 'w') as out:
        for i, enst_raw in enumerate(te_enst_list):
            enst = enst_raw.split('.')[0]  # strip version
            if enst in pep_by_enst:
                ...
                # Write dummy entry to preserve index alignment
                out.write(f'>{i}|{enst}|MISSING\n')
                missing += 1
