# -*- coding: utf-8 -*-
# ============================================================================
# v8-3_integrated_full_model.py
#
# 핵심 발견 (2026-05-06) — LinAUPRC Oracle:
#   두 가지 독립된 병목 정량화:
#   [병목1] Phase 2가 Type-B embedding 손상 (GO:0006941: Ph1=0.1825→Ph2=0.1554, -0.027)
#   [병목2] Head gap: sigmoid head(focal α=0.10)가 embedding 정보 추출 불충분
#           GO:0007204: Ph2 LinAUPRC=0.2230 vs Model=0.1462 (gap +0.077)
#           GO:0030017: Ph2 LinAUPRC=0.2805 vs Model=0.1787 (gap +0.102)
#
# v8-3 전략:
#   Type-B: Phase 1 → Phase 2 SKIP → Phase 3 (Balanced LR on Ph1 train emb)
#   Type-A: Phase 1 → Phase 2 (v8b 동일) → sigmoid head
#
# 변경 사항 (v8b 대비):
#   [v8-3-1] Phase 1 완료 후 TRAIN embedding 추출 & 저장 (balanced LR 학습용)
#   [v8-3-2] Type-B: Phase 2 완전 SKIP
#   [v8-3-3] Type-B: Phase 3 = LogisticRegression(class_weight='balanced', C=1.0)
#             train embedding으로 학습 → test embedding으로 최종 예측
#   [v8-3-4] Final score: Type-B = balanced LR score, Type-A = sigmoid head (v8b)
#
# AP6 위험 없음:
#   AP6는 TF feature_model frozen 상태에서 neural head 재학습 → sep_ratio 급락.
#   v8-3 Phase 3는 sklearn LR만 사용, TF 모델 weight 변경 없음.
# ============================================================================
# (원본: v8b_integrated_full_model.py — v7c + Phase 2 Unified Loss)
#
...
if IS_TYPE_B:
    # ------------------------------------------------------------------
    # [v8-3-3] Type-B: Phase 3 — Balanced LR on Phase 1 train embeddings
    # ------------------------------------------------------------------
    print("\n" + "=" * 58)
    print(">>> PHASE 3 [v8-3]: Balanced LR — Phase 2 SKIPPED for Type-B")
    print("    근거: Phase 2가 Type-B embedding 손상 (F16)")
    print("          Head gap: balanced LR이 sigmoid head보다 우수 (F17)")
    print("=" * 58)

    test_emb_ph1 = emb_by_phase['Ph1(contrastive)']
    n_pos_tr = int((y_train == 1).sum())
    n_neg_tr = int((y_train == 0).sum())
    pos_weight_lr = n_neg_tr / max(n_pos_tr, 1)
    print("  Train: n_pos={}, n_neg={}, pos_weight={:.1f}".format(
        n_pos_tr, n_neg_tr, pos_weight_lr))
    print("  Test emb: {}  y_test: pos={} neg={}".format(
        test_emb_ph1.shape, int(y_test.sum()), int((1-y_test).sum())))

    clf_balanced = LogisticRegression(
        class_weight='balanced', C=1.0, max_iter=1000, solver='lbfgs')
    clf_balanced.fit(train_emb_ph1, y_train)
    balanced_proba = clf_balanced.predict_proba(test_emb_ph1)[:, 1]
    ...
    bal_auprc = average_precision_score(y_test, balanced_proba) \
                if y_test.sum() > 0 else float('nan')
    print("\n  [Phase 3] Balanced LR: AUROC={:.4f}  AUPRC={:.4f}".format(
        bal_auroc, bal_auprc))
    print("  [Phase 3] Comparison — v8b sigmoid: AUPRC~0.1177 (GO:0006941)")
