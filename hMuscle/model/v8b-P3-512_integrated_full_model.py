# -*- coding: utf-8 -*-
# ============================================================================
# v8b-P3-512_integrated_full_model.py  (v8b + P3 + EMB_DIM 64→512)
#
# P3 변경사항 (v8b 대비):
...
#
# +512 변경사항 (P3-256 대비):
#   ESM-2 projection bottleneck 확장: 256-dim → 512-dim
#   근거: esm2_dim_sweep.py 결과 — PCA 256d=64%, PCA 512d=91% (+42%)
#         SP✗ 조건에서 dim 효과가 더 큼 (SP×dim 음의 상호작용 확인)
#   변경 내역:
#     esm2_d1:       Dense(256) → Dense(512)
#     esm2_d2:       Dense(256) → Dense(512)
#     esm2_feat:     Dense(256) → Dense(512)   ← 핵심 bottleneck
#     head_dense256: Dense(256) → Dense(512)   이름: head_dense512
#     emb_dim:       256 → 512
#     concat:        [256+32+16+16=320] → [512+32+16+16=576]
#   목적: P3-256 대비 추가 bottleneck 해소, Type-B GO term gap 축소
#
# ============================================================================
...
HEAD_DENSE_NAMES      = {'head_dense512'}          # [P3-512] embedding geometry — slow LR
...
def prototype_warm_init(base_model, embeddings_np, y_np, proto_fn=None,
    ...
    for layer in base_model.layers:
        if layer.name == 'prediction_out':
            w_init = (w_dir * scale).reshape(512, 1).astype(np.float32)  # [P3-512] 64→512
...
x_esm = Dense(512, kernel_regularizer=regularizers.l2(1e-5), name='esm2_d1')(esm2_input)  # [P3-512] 256→512
x_esm = Activation('relu')(x_esm)
x_esm = Dropout(0.2)(x_esm)
x_esm = Dense(512, kernel_regularizer=regularizers.l2(1e-5), name='esm2_d2')(x_esm)  # [P3-512] 256→512
x_esm = Activation('relu')(x_esm)
esm2_feat  = Dense(512, activation='relu', kernel_regularizer=regularizers.l2(1e-5),
                   name='esm2_feat')(x_esm)  # [P3-512] 256→512 (핵심 bottleneck 해소)
...
x = Dense(512, kernel_regularizer=regularizers.l2(1e-5), name='head_dense512')(concat)  # [P3-512] 256→512
x = Activation('relu')(x)
x = Dropout(0.2)(x)
...
if IS_TYPE_B:
    ...
    proto_loss_fn = PrototypeContrastiveLoss(
        n_prototypes=PROTO_K, emb_dim=512,  # [P3-512]
