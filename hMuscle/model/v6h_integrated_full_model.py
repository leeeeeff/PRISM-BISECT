# -*- coding: utf-8 -*-
# ============================================================================
# v6h_integrated_full_model.py  (v6-hybrid)
#
# v6 대비 핵심 변경:
#
...
#   Fusion       : concat[64+32+16=112] → Dense(48,relu) → L2_norm → emb[48]
#                  → Dense(1,sigmoid)
#   EMB_DIM=112
#
# [TRAIN] 단계별 가중치 갱신 범위
#   Phase 1  (triplet, GradientTape): ESM-2 ✅ | CNN ❌(frozen) | Domain ✅
#   Phase 1.5(focal, encoder frozen): ESM-2 ❌ | CNN ❌         | Domain ❌
#   Phase 2  (focal only):            ESM-2 ❌(frozen) | CNN ✅ | Domain ✅
...
try:
    ...
    print("Usage: python v6h_integrated_full_model.py <GO_TERM>")
    sys.exit(1)
...
# ESM-2 / CNN 레이어 이름 (단계별 freeze 제어용)
...
def phase1_triplet_epoch_hybrid(
    ...
    """
    [HYBRID] Phase 1: ESM-2 + Domain 학습 (CNN frozen)
...
# [8-v6h] 데이터 로딩
# --------------------------------------------------------------------------
print('>>> Preparing Data (v6h/Hybrid) for ' + selected_go)
...
print("\n[Model] ESM-2(640→256→128→64) + CNN(1500→Conv×2→Pool→32) + Domain(LSTM 16) → concat[112]")
print("[Model] Head: Dense(48) → L2_norm → embedding[48] → sigmoid")
print("[Model] Phase 1: ESM-2✅ CNN❌ Domain✅ | Phase 2: ESM-2❌ CNN✅ Domain✅")
...
# CNN frozen (Phase 1 동안 ESM-2 + Domain만 학습)
set_cnn_trainable(feature_model, False)
print("  [Phase1] CNN layers frozen:", CNN_TRAINABLE_NAMES)
...
# ==========================================================================
print("\n" + "=" * 55)
print(">>> PHASE 2: CNN Fine-tuning")
print("    [HYBRID] ESM-2 FROZEN | CNN+Domain TRAIN | Focal only (no triplet)")
print("    [I5] AUPRC early stop [R9.1]")
print("=" * 55)
...
print("  [Phase2] ESM-2 layers frozen:", ESM2_TRAINABLE_NAMES)
print("  [Phase2] CNN + Domain layers: trainable")
