# -*- coding: utf-8 -*-
# ============================================================================
# v8b-P3-256_integrated_full_model.py  (v8b + P3 + EMB_DIM 64→256)
#
# P3 변경사항 (v8b 대비):
...
#   근거: esm2_dim_sweep.py 결과 — PCA 64d=35%, PCA 256d=64%, PCA 512d=91%
#         640→64 압축이 전체 gap의 ~67% 설명
#   변경 내역:
#     esm2_d2:      Dense(128) → Dense(256)
#     esm2_feat:    Dense(64)  → Dense(256)   ← 핵심 bottleneck
#     head_dense64: Dense(64)  → Dense(256)   이름: head_dense256
#     emb_dim:      64 → 256
#     concat:       [64+32+16+16=128] → [256+32+16+16=320]
#   목적: 파이프라인 embedding capacity 확장으로 ESM-2 정보 손실 감소
#
# ============================================================================
...
HEAD_DENSE_NAMES      = {'head_dense256'}          # [P3-256] embedding geometry — slow LR
...
def prototype_warm_init(base_model, embeddings_np, y_np, proto_fn=None,
    ...
    for layer in base_model.layers:
        if layer.name == 'prediction_out':
            w_init = (w_dir * scale).reshape(256, 1).astype(np.float32)  # [P3-256] 64→256
...
x_esm = Dense(256, kernel_regularizer=regularizers.l2(1e-5), name='esm2_d2')(x_esm)  # [P3-256] 128→256
x_esm = Activation('relu')(x_esm)
esm2_feat  = Dense(256, activation='relu', kernel_regularizer=regularizers.l2(1e-5),
                   name='esm2_feat')(x_esm)  # [P3-256] 64→256 (핵심 bottleneck 해소)
...
x = Dense(256, kernel_regularizer=regularizers.l2(1e-5), name='head_dense256')(concat)  # [P3-256] 64→256
x = Activation('relu')(x)
x = Dropout(0.2)(x)
...
if IS_TYPE_B:
    ...
    proto_loss_fn = PrototypeContrastiveLoss(
        n_prototypes=PROTO_K, emb_dim=256,  # [P3-256]
